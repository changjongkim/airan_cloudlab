...
requal="$raw/confirm36_requalification_job${SLURM_JOB_ID}.json"
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    ...
    --seed 20321937 --snr-db -8.5 --channel-seed-base 20326900
python3 - "$requal" <<'PY'
import json, sys
data = json.load(open(sys.argv[1]))
if len(data["records"]) != 200 or data["deadline_misses"] != 0:
    raise SystemExit("confirm36 operational requalification failed")
print("confirm36 operational requalification passed")
PY
sleep 20

SOFTWALL_GPU_LOCK_HELD=1 ENDPOINT_CAP=80 PERIOD_MS=90 DEADLINE_MS=80 \
ENDPOINT_TIMEOUT_MS=30 WARMUP=20 SEED=20321036 SNR_DB=-8.5 \
CHANNEL_SEED_BASE=20326000 \
    bash "$SOFTWALL_SCRIPTS/run_same_request_ipc.sh" \
    confirm36_same_request_natural_s2 10000
sleep 20

SOFTWALL_GPU_LOCK_HELD=1 ITERATIONS=10000 PERIOD_MS=90 DEADLINE_MS=80 \
SNR_DB=-8.5 SEED=20321036 CHANNEL_SEED_BASE=20326000 QUIET_SECONDS=20 \
    bash "$SOFTWALL_SCRIPTS/run_external_endpoint_baselines.sh" \
    confirm36_same_node_baselines

shifter_gpu python3 /softwall/analyze_same_request_ipc.py \
    --raw "$raw" --campaign confirm36_same_request_natural_s2 \
    --output "$result/confirm36_same_request_natural_s2.md"

external="$raw/confirm36_same_request_natural_s2_cap80_job${SLURM_JOB_ID}_controller.json"
external_worker="$raw/confirm36_same_request_natural_s2_cap80_job${SLURM_JOB_ID}_worker.json"
shifter_gpu python3 /softwall/analyze_external_endpoint_baselines.py \
    --raw "$raw" --campaign confirm36_same_node_baselines \
    --external-controller "$external" \
    --external-worker "$external_worker" \
    --protocol "$result/confirm36_same_node_full_comparison_protocol.json" \
    --output "$result/confirm36_same_node_baselines.md" --margin-pp 0.25

cleanup
trap - EXIT INT TERM
echo "confirm36 same-node full comparison completed"
