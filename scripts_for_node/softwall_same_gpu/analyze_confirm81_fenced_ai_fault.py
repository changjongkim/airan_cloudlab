...
"""Frozen audit of one fenced AI RPC timeout and RAN continuation."""
...
def main() -> None:
    ...
    protocol = read(base / "confirm81_fenced_fault_protocol.json")
    raw = base / "raw"
    ...
    marker = read(raw / f"{prefix}_completion_marker.json")
    records = ran["records"]
    ...
    fault_rows = [row for row in ran["background_faults"]
                  if row["type"] == "RPCResponseTimeoutFenced"]
    fenced_records = [row for row in ran["background_records"]
                      if row.get("fault_fence_confirmed")]
    fenced_actions = [row for row in ran["recovery_decisions"]
                      if row.get("ai_rpc_timeout_fenced")]
    fault_release = fault_rows[0]["release_index"] if len(fault_rows) == 1 else None
    gates = {
        "source_hashes": all(
        ...
        "contract": (
            ...
            and background["joint_response_delay_ms"]
                == protocol["worker_joint_response_delay_ms"]
            and background["completion_marker"]
                == str(raw / f"{prefix}_completion_marker.json")
        ...
        "joint_physical_path": (
            len(joint_ai) == protocol["expected_joint_ai"]
        ...
        "fenced_fault_continuation": (
            len(ran["background_faults"]) == len(fault_rows) == 1
            and len(fenced_records) == len(fenced_actions) == 1
            and ran["fault_fenced_ai_units"] == 1
            and ran["background_enabled_final"] is False
            and fault_release is not None
            and fault_release <= protocol["latest_fault_release"]
            and protocol["iterations"] - fault_release - 1
                >= protocol["min_releases_after_fault"]
            and all(row["release_index"] <= fault_release
                    for row in ran["background_records"])
            and fault_rows[0]["lease_id"] == marker["lease_id"]
                == fenced_records[0]["marker_lease_id"]
            and marker["completed_ns"] == fault_rows[0]["gpu_completed_ns"]
                == fenced_records[0]["gpu_completed_ns"]
            and marker["completed_ns"] < fault_rows[0]["detected_ns"]
                <= fenced_records[0]["returned_ns"]
            and marker["host"] == ran["host"]
            and str(marker["slurm_job_id"]) == protocol["job"]
            and ran["ai_completion_marker_path"]
                == str(raw / f"{prefix}_completion_marker.json")
            and all(row["commit_kind"] in ("nrx", "conventional")
                    and not row["deadline_miss"]
                    for row in records[4 * (fault_release + 1):])
        ) if fault_release is not None and len(fenced_records) == 1
            and len(fenced_actions) == 1 else False,
    }
    report = {
        "schema": "softwall-confirm81-fenced-ai-timeout-v1",
        ...
        "releases_after_fault": (None if fault_release is None else
                                 protocol["iterations"] - fault_release - 1),
        "fault_fenced_ai_units": ran["fault_fenced_ai_units"],
        "max_ai_host_ms": max(row["execution_ms"] for row in ran["background_records"]),
        "interpretation": "A GPU-synchronized worker marker certifies one joint AI unit after its RPC response is delayed beyond the 12 ms timeout. The controller retires that lease, disables future AI admissions and continues four-cell radio releases. Only post-GPU-response delay is tested; no GPU hang/worker crash proof, WCET, real DU expiry or joint-policy superiority claim.",
    }
    output = base / f"confirm81_fenced_fault_job{protocol['job']}.json"
    ...
    print(json.dumps({
        ...
        "joint_ai_units": ran["ai_before_recovery_units"],
        "fault_fenced_ai_units": ran["fault_fenced_ai_units"],
        "releases_after_fault": report["releases_after_fault"],
    }, indent=2))
