...
endpoint_delay_sequence=${ENDPOINT_DELAY_SEQUENCE:-0}
endpoint_delay_ms=${ENDPOINT_DELAY_MS:-0}
ai_response_delay_unit=${AI_RESPONSE_DELAY_UNIT:-0}
ai_response_delay_ms=${AI_RESPONSE_DELAY_MS:-0}
inject_neural_failure_index=${INJECT_NEURAL_FAILURE_INDEX:--1}
...
shifter_gpu env \
    ...
    --delay-sequence "$endpoint_delay_sequence" \
    --delay-ms "$endpoint_delay_ms" \
...
if [[ -n "$background_kind" ]]; then
    shifter_gpu env \
        ...
        CUDA_MPS_CLIENT_PRIORITY=1 \
        python3 /softwall/ai_worker_fault_probe.py \
        --kind "$background_kind" --mode rpc \
        --repeats "$background_repeats" --socket "$background_socket" \
        --response-delay-unit "$ai_response_delay_unit" \
        --response-delay-ms "$ai_response_delay_ms" \
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/same_request_transaction_controller_fault_probe.py \
    --tag "$tag" --ipc-dir "$state" \
    --engine /softwall_runtime/engines/neural_rx_fp16_full.trt \
    ...
    --inject-neural-failure-index "$inject_neural_failure_index" \
    "${controller_args[@]}"
