...
requal="$raw/confirm45_requalification_job${SLURM_JOB_ID}.json"
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    ...
    --seed 20321945 --snr-db -8.5 --channel-seed-base 20335900
python3 - "$requal" <<'PY'
import json, sys
data = json.load(open(sys.argv[1]))
if data['iterations'] != 200 or data['deadline_misses']:
    raise SystemExit('confirm45 operational requalification failed')
print('confirm45 operational requalification passed')
PY

for round in 1 2; do
    case "$round" in
        1) seed=20321045; channel=20335000; order="eager external" ;;
        2) seed=20321046; channel=20336000; order="external eager" ;;
    esac
    for condition in $order; do
        ...
        if [[ "$condition" == eager ]]; then
            ...
            AI_RPC_TIMEOUT_MS=45 QUIET_SECONDS=0 \
                bash "$SOFTWALL_SCRIPTS/run_s2_campaign.sh" \
                1 10000 "confirm45_r${round}_eager_job${SLURM_JOB_ID}"
        else
            ...
            SNR_DB=-8.5 CHANNEL_SEED_BASE="$channel" \
                ...
                "confirm45_r${round}_external" 10000
        fi
    done
done

shifter_gpu python3 /softwall/analyze_transaction_vs_eager.py \
    --raw "$raw" --job "$SLURM_JOB_ID" \
    --protocol "$result/confirm45_transaction_vs_eager_protocol.json" \
    --output "$result/confirm45_transaction_vs_eager.md"

cleanup
trap - EXIT INT TERM
echo 'confirm45 transactional external vs eager completed'
