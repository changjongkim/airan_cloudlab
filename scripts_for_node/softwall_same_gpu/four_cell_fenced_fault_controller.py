#!/usr/bin/env python3
"""Four-cell atomic AI lease with worker-side GPU fence fault continuation.
...
def main() -> None:
    ...
    try:
        ...
        args.completion_marker.unlink(missing_ok=True)
        background = FencedBackgroundClient(args.socket, args.ai_rpc_timeout_ms)
        ...
        for index in range(args.iterations):
            ...
            if (background.enabled
                    and any(tx.reservation is not None for tx in transactions)):
            ...
            if pending:
                ...
                if (background.enabled and
                    ...
                    if joint is not None:
                        ...
                        prior_count = len(background.records)
                        fenced = background.run_joint(
                            index, ai_deadline_ns, args.ai_budget_ms,
                            lease.unit_id, args.completion_marker,
                            earliest_recovery_ns, ai_guard_ns,
                        )
                        if not fenced or len(background.records) != prior_count + 1:
                        ...
                        recovery_action["ai_rpc_timeout_fenced"] = ai_record.get(
    ...
    result = {
        "schema": "softwall-four-cell-fenced-ai-fault-v1",
        ...
        "fault_fenced_ai_units": sum(
            row.get("fault_fence_confirmed", False) for row in background_records
        ),
        "ai_timely_units": sum(
        ...
        "background_enabled_final": background.enabled if background is not None else None,
        "ai_completion_marker_path": str(args.completion_marker),
    ...
    print(
        f"[FOUR-CELL/FENCED-FAULT] correct={result['correct_cells']}/{len(records)} "
