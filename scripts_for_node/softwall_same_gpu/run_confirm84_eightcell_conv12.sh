...
# Confirm84: exploratory eight-cell B_conv12 service envelope.
...
prefix=${SOFTWALL_GATE_PREFIX:-confirm84_eightcell_seed${channel_seed_base}_job${SLURM_JOB_ID}}
...
print('confirm84 operational requalification passed', flush=True)
PY
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/eight_cell_bound_probe_controller.py \
    --tag-prefix "$tag" --ipc-dir "$state" \
    --engine /softwall_runtime/engines/neural_rx_fp16_full.trt \
    --socket "$background_socket" \
    --output "$raw/${prefix}_controller.json" \
    --iterations "$iterations" --cells 8 --warmup 20 \
    --period-ms 180 --deadline-ms 155 \
    --nrx-bound-ms 50 --conv-bound-ms 12 --commit-guard-ms 2 \
...
cleanup
trap - EXIT INT TERM
echo "confirm84 B_conv12 eight-cell run completed"
