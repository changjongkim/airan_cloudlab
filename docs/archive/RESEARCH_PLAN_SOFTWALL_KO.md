...
| NRx 처리율 | full A100 1130.516 req/s (direct TRT + CUDA Graph, 평균 0.882ms, p99 1.107ms); 4g MIG 1 replica는 745.103 req/s, 평균 1.340ms | NRX_CAPACITY.csv의 서로 다른 두 행 |
...
| 과거 same-GPU MIG P2P 전송 비용 | 1.4MB 구성의 `transport_us=76.84μs`; 단일 물리 GPU 안의 MIG pair 측정이며 cross-GPU NVLink 수치가 아님 | PLACEMENT_SUMMARY.csv의 `Cross P2P` 행 |
