...
class FallbackCalendar:
    ...
    def __init__(
        ...
        self._joint_leases: set[LeaseReservation] = set()
    ...
    def _overlaps_joint_lease(self, start_ns: int, finish_ns: int) -> bool:
        return any(
            start_ns < lease.predicted_finish_ns and lease.start_ns < finish_ns
            for lease in self._joint_leases
        )
    ...
    def reserve(self, request: DartRequest) -> Optional[FallbackReservation]:
        ...
        with self._lock:
            ...
            for lane in range(self.capacity):
                if self._overlaps_joint_lease(start_ns, finish_ns):
                    return None
                conflict = any(
                    current.lane == lane
    ...
    def replan_atomic(
        ...
        if not proposed or now_ns < 0:
            return None
        with self._lock:
            candidate = self._stage_replan_locked(proposed, now_ns)
            if candidate is None:
                return None
            staged, replacements = candidate
            self._reservations = staged
            return replacements

    def _stage_replan_locked(self, proposed, now_ns):
        """Build a candidate while holding the calendar lock, without publishing it."""
        ...
        for request, old, start_ns, lane in proposed:
            ...
            if (
                ...
                or finish_ns > request.deadline_ns - self.commit_guard_ns
                or self._overlaps_joint_lease(start_ns, finish_ns)
            ):
                return None
            replacement = FallbackReservation(
                slot_id=request.slot_id, epoch=request.epoch, lane=lane,
                start_ns=start_ns, predicted_finish_ns=finish_ns,
            )
    ...
    ) -> Optional[Tuple[Dict[Tuple[int, int], FallbackReservation], LeaseReservation]]:
        """Publish a recovery replan only if its AI lease also fits.

        All active fallback credits precede an AI lease only when their first
        reserved start remains after the lease's bounded finish plus guard.
        The controller must retire this lease only after a GPU completion fence;
        a timed-out or still-running unit continues to block recovery retiming.
        """
        if now_ns < 0 or qmax_ns <= 0 or drain_guard_ns < 0:
            return None
        proposed = list(moves)
        with self._lock:
            candidate = self._stage_replan_locked(proposed, now_ns)
            ...
            earliest_recovery_ns = min(
                (reservation.start_ns for reservation in staged.values()),
                default=now_ns + qmax_ns,
            )
            next_guard_ns = min(earliest_recovery_ns, now_ns + qmax_ns)
            lease = endpoint.reserve_lease(
                now_ns, next_guard_ns, drain_guard_ns, units
            )
            if lease is None:
                return None
            self._joint_leases.add(lease)
            self._reservations = staged
            return replacements, lease

    def release_joint_lease(
        self, lease: LeaseReservation, gpu_fence_confirmed: bool
    ) -> bool:
        """Do not free an AI blocking interval on timeout or logical cancel."""
        if not gpu_fence_confirmed:
            return False
        with self._lock:
            if lease not in self._joint_leases:
                return False
            self._joint_leases.remove(lease)
            return True
    ...
    def snapshot(self) -> dict:
        with self._lock:
            ...
            return {
                ...
                "joint_leases_outstanding": len(self._joint_leases),
                "by_lane": by_lane,
            }
...
class DartRuntime:
    ...
    def replan_and_lease(
        ...
        """Commit a safe recovery calendar and bounded AI unit together.

        The caller must serialize physical fallback dispatch with this decision
        and retain the lease until its GPU completion fence is observed.
        """
        if self.fallback_calendar is None or endpoint_id not in self.endpoints:
            return None
        plan = list(moves)
        if len({id(tx) for tx, _, _ in plan}) != len(plan):
            return None
        transactions = sorted((tx for tx, _, _ in plan), key=id)
        for transaction in transactions:
            transaction._lock.acquire()
        try:
            if any(
                ...
                self.metrics["joint_admission_rejected"] += 1
                return None
            proposal = [
                (tx.request, tx.fallback_reservation, start_ns, lane)
                for tx, start_ns, lane in plan
            ]
            result = self.fallback_calendar.replan_and_lease_atomic(
                proposal, self.endpoints[endpoint_id], now_ns,
                qmax_ns, drain_guard_ns, units,
            )
            if result is None:
                self.metrics["joint_admission_rejected"] += 1
                return None
            replacements, lease = result
            for tx, _, _ in plan:
                tx.fallback_reservation = replacements[(tx.request.slot_id, tx.request.epoch)]
            self.metrics["joint_admission_admitted"] += 1
            self.metrics["fallback_replanned"] += len(plan)
            return replacements, lease
        finally:
            for transaction in reversed(transactions):
    ...
    def retire_joint_lease(
        self, lease: LeaseReservation, gpu_fence_confirmed: bool
    ) -> bool:
        if self.fallback_calendar is None:
            return False
        released = self.fallback_calendar.release_joint_lease(
            lease, gpu_fence_confirmed
        )
        self.metrics["joint_lease_retired" if released else "joint_lease_retained"] += 1
        return released
