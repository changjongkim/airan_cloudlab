...
Diagnostic of one bounded AI unit while accepted NeuralRx requests are unresolved.
...
def main() -> None:
    ...
    parser.add_argument("--ai-during-nrx", choices=("off", "one"), required=True)
    ...
    try:
        ...
        for index in range(args.iterations):
            ...
            if args.ai_during_nrx == "one" and any(tx.reservation is not None for tx in transactions):
                ...
                if time.perf_counter_ns() + ai_budget_ns + ai_guard_ns <= earliest_cutoff_ns:
                    ...
                    if not background.enabled or len(background.records) != prior_count + 1:
                        raise RuntimeError("pre-radio AI physical completion unconfirmed; stop before recovery")
                    ...
                    if ai_record["fallback_guard_violation"]:
                        raise RuntimeError("pre-radio AI finished after earliest recovery guard")
