...
# Confirm60: one low-feature failure-aware baseline arm.
...
mode=${2:?mode must be always or low_threshold}
case "$pair:$mode" in
    1:always|1:low_threshold|2:always|2:low_threshold) ;;
    *) echo "invalid pair/mode: $pair:$mode" >&2; exit 2 ;;
esac
if [[ "$pair" == 1 ]]; then
    payload_seed=20530001
    channel_seed_base=20531000
else
    payload_seed=20540002
    channel_seed_base=20541000
fi
iterations=${SOFTWALL_GATE_ITERATIONS:-1000}
prefix=${SOFTWALL_GATE_PREFIX:-confirm60_pair${pair}_${mode}_job${SLURM_JOB_ID}}
...
if x['iterations'] != 200 or x['deadline_misses']:
    raise SystemExit('confirm60 operational requalification failed')
print('confirm60 operational requalification passed', flush=True)
PY
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/two_endpoint_low_gate_controller.py \
    ...
    --gate-mode "$mode" --gate-threshold 1.9490545988082886
...
cleanup
trap - EXIT INT TERM
echo "confirm60 pair=$pair mode=$mode completed"
