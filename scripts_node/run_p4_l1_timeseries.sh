#!/usr/bin/env bash
# P4: L1 latency time-series during AI co-tenant lifecycle.
# Goal: see if disturbance is burst, ramp, or steady.
# Method: run L1 with ITERS=500 (~10x normal) while AI runs in background.
# real_l1.py records raw_ms[]; we save JSONs for time-series analysis.
...
GPU="${GPU:-0}"
N="${N:-3}"
ITERS_L1="${ITERS_L1:-500}"      # 10x normal → ~15s of L1 work
CELLS_L1="${CELLS_L1:-20}"
AI_DURATION="${AI_DURATION:-30}"   # AI runs longer than L1 ITERS
...
OUT_ROOT="$HOME/cloudlab_aerial/results/$DATE_DIR/p4_l1_timeseries"
...
# Use split-50-50 (3g+3g) layout for all P4 — L1 on first 3g, AI on second 3g
reconfigure_mig "9,9"
l1_uuid=$(uuid_for_profile "3g.20gb")
ai_uuid=$(uuid_for_profile_nth "3g.20gb" 2)
log "L1=$l1_uuid  AI=$ai_uuid (3g+3g layout)"

for scenario in alone qwen_small chanpred sat_compute neuralrx xapp resnet forecaster sat_hbm; do
  ...
  for i in $(seq 1 $N); do
    ...
    if [[ "$scenario" == "alone" ]]; then
      run_l1_long "$l1_uuid" "$outdir/run_${i}_l1.log" "P4_${scenario}_run${i}"
    else
...
log "p4 timeseries DONE — raw_ms arrays in JSON in $OUT_ROOT"
