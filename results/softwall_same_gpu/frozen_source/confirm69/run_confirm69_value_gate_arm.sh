...
# Confirm69: train-derived conditional-PHY-value gate against tuned low gate.
...
require_allocation
mode=${1:?mode must be low_threshold or value_bin}
case "$mode" in low_threshold|value_bin) ;; *) echo "invalid mode: $mode" >&2; exit 2 ;; esac
payload_seed=${SOFTWALL_PAYLOAD_SEED:-20690001}
channel_seed_base=${SOFTWALL_CHANNEL_SEED_BASE:-20691000}
iterations=${SOFTWALL_GATE_ITERATIONS:-500}
prefix=${SOFTWALL_GATE_PREFIX:-confirm69_${mode}_seed${channel_seed_base}_job${SLURM_JOB_ID}}
if [[ "$mode" == low_threshold ]]; then
    gate_threshold=1.9490545988082886
else
    gate_threshold=1.9430251121520996
fi
...
worker1_pid=""
background_pid=""
tag="b${mode:0:1}_${SLURM_JOB_ID}_${BASHPID}"
...
if x['iterations'] != 200 or x['deadline_misses']:
    raise SystemExit('confirm69 operational requalification failed')
print('confirm69 operational requalification passed', flush=True)
PY
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/two_endpoint_value_baseline_controller.py \
    --tag-prefix "$tag" --ipc-dir "$state" \
    --engine /softwall_runtime/engines/neural_rx_fp16_full.trt \
    --socket "$background_socket" \
    --output "$raw/${prefix}_controller.json" \
    --iterations "$iterations" --warmup 20 --period-ms 150 --deadline-ms 130 \
    --nrx-bound-ms 50 --conv-bound-ms 25 --commit-guard-ms 2 \
    --endpoint-timeout-ms 100 --ai-budget-ms 40 --ai-guard-ms 2 \
    --ai-rpc-timeout-ms 35 --seed "$payload_seed" --snr-db -8.5 \
    --channel-seed-base "$channel_seed_base" \
    --gate-mode "$mode" --gate-threshold "$gate_threshold" \
    --gate-upper 2.0467076301574707 \
    --early-mandatory on --alternate-admission-order
...
cleanup
trap - EXIT INT TERM
echo "confirm69 mode=$mode completed"
