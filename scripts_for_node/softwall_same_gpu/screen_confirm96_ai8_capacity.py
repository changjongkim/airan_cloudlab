...
"""Contingent AI8 model check on the fixed independent Confirm94 PHY trace."""
...
def main() -> None:
    ...
    protocol = read(base / "confirm96_ai8_capacity_protocol.json")
    source_ok = all(
    ...
    if not physical["all_pass"] or physical["job"] != protocol["physical_job"]:
    ...
    radio_differences = [row["joint_radio_gain"] - row["staged_radio_gain"]
                         for row in feasible]
    report = {
        "schema": "softwall-confirm96-ai8-capacity-model-check-v1",
        ...
        "joint_ai_above_max_radio": sum(
            row["joint_ai"] > row["max_radio_ai"] + 1e-9 for row in feasible),
        "all_policies_complete_all_seven": all(
            all(abs(row[name] - protocol["ai_units"]) < 1e-9
                for name in ("joint_ai", "joint_greedy_ai", "staged_ai", "max_radio_ai"))
            for row in feasible),
        ...
        "max_radio_actual_correct": sum(row["max_radio_actual_correct"]
                                        for row in feasible),
        "records": records,
        "interpretation": "Deterministic duration sensitivity on the already-used independent Confirm94 synthetic PHY trace, contingent on Confirm95 physical AI8 mode qualification. It checks whether the former seven-AI all-fail capacity pressure survives when each AI unit has an 8-ms host budget. This is not a fresh independent PHY replication, not seven atomic GPU leases, and not a hard real-time proof.",
    }
    output = base / "confirm96_ai8_capacity_screen.json"
    ...
    print(json.dumps({key: report[key] for key in (
        ...
        "joint_ai_above_max_radio", "all_policies_complete_all_seven",
