...
# One local-IPC plus two remote-P2P SoftWall arm.
...
label=${SOFTWALL_MGPU_LABEL:-g4_three_endpoint_job${SLURM_JOB_ID}}
system=softwall
...
rm -f "$state"/cuda_ipc_* "$socket" "$controller_output" \
    "$local_worker_output" "$remote1_worker_output" "$remote2_worker_output" "$background_output" \
...
value={
 ...
 "mps":True, "mps_devices":[0,1,2], "local_nrx_cap":40,
 "remote_nrx_cap_each":40, "qwen_cap":20, "nrx_endpoints":3,
...
local_pid=""; remote1_pid=""; remote2_pid=""; background_pid=""
cleanup() {
    for pid in "$local_pid" "$remote1_pid" "$remote2_pid" "$background_pid"; do
...
shifter_local_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    ...
    --output "$controller_output" --iterations "$iterations" --cells 4 --nrx-endpoints 3 \
...
python3 - "$protocol" "$controller_output" "$local_worker_output" \
    "$remote1_worker_output" "$remote2_worker_output" "$background_output" "$requal_output" "$result" <<'PY'
import hashlib, json, os, sys
from pathlib import Path

protocol,controller,local,remote1,remote2,background,requal,out=map(Path,sys.argv[1:])
p=json.loads(protocol.read_text()); c=json.loads(controller.read_text())
l=json.loads(local.read_text()); r1=json.loads(remote1.read_text()); r2=json.loads(remote2.read_text())
...
gates={
 ...
 "remote_sequence":bool(r1["sequences_contiguous"]) and bool(r2["sequences_contiguous"]),
...
result={
 "schema":"softwall-multigpu-three-endpoint-arm-gate-v1",
 "protocol":str(protocol), "controller":str(controller),
 "local_worker":str(local), "remote1_worker":str(remote1), "remote2_worker":str(remote2),
 ...
 "artifact_sha256":{str(x):hashlib.sha256(x.read_bytes()).hexdigest() for x in [protocol,controller,local,remote1,remote2,background,requal]},
 "scope":"one local IPC plus two remote P2P NRx endpoints in the four-cell SoftWall/Qwen/fault path",
}
tmp=out.with_suffix(out.suffix+'.tmp'); tmp.write_text(json.dumps(result,indent=2)); tmp.replace(out)
print(json.dumps(result,indent=2))
if not result['all_pass']: raise SystemExit('three-endpoint SoftWall arm failed')
PY

cleanup
trap - EXIT INT TERM
echo "three-endpoint SoftWall arm complete: $result"
