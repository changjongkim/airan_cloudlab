#!/usr/bin/env python3
"""Repeated V17.1 coordinator/shared cuPHY worker for the C158 pilot."""
...
def main() -> None:
    ...
    # Exercise every response P2P direction and compile the byte-comparison
    # monitor before readiness. The owner has not published a request, so the
    # temporary response bytes are not consumable radio state.
    preflight_echo = []
    for peer, context in zip(peers, contexts):
        with cp.cuda.Device(args.destination_device):
            context["remote_backward"].fill(cp.uint8(0xA5))
    ...
    if not all(preflight_echo):
        raise RuntimeError("recovery response echo preflight failed")
    ...
    recoveries = []
    result = {
        "schema": "softwall-actual-nrx-repeated-coordinator-v1",
        "host": platform.node(),
    ...
    try:
        ...
        atomic_json(args.schedule_file, {
            "schema": "softwall-actual-nrx-repeated-schedule-v1",
            "first_release_wall_ns": first_release_ns,
            "period_ns": period_ns,
            "iterations": args.iterations,
            "release_semantics": "synthetic t_IQ_ready after validation preparation",
            "staging_semantics": (
                "next synthetic input and validation oracle start only after "
        ...
        for index in range(args.iterations):
            ...
            dispatch = {
                "schema": "softwall-actual-nrx-repeated-dispatch-v1",
                "sequence": sequence,
