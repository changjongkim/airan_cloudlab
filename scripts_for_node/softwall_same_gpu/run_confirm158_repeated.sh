...
# C158 repeated actual TensorRT NeuralRx -> V17.1 -> Qwen/shared recovery.
...
label=${SOFTWALL_C158_LABEL:-confirm158a_repeated_job${SLURM_JOB_ID}}
campaign=${SOFTWALL_C158_CAMPAIGN:-development}
seed_base=${SOFTWALL_C158_SEED_BASE:-26000000}
excluded_nodes=${SOFTWALL_C158_EXCLUDED_NODES:-nid001109,nid001085,nid001064}
iterations=${SOFTWALL_C158_ITERATIONS:-250}
period_ms=${SOFTWALL_C158_PERIOD_MS:-600}
warmup=${SOFTWALL_C158_WARMUP:-10}
release_lead_ms=${SOFTWALL_C158_RELEASE_LEAD_MS:-3000}
release_prewarm_lead_ms=${SOFTWALL_C158_PREWARM_LEAD_MS:-100}
...
python3.11 "$SOFTWALL_SCRIPTS/build_confirm158_repeated_protocol.py" \
    ...
    --release-prewarm-lead-ms "$release_prewarm_lead_ms"

current_node=$(hostname)
case ",$excluded_nodes," in
    *",$current_node,"*)
        echo "C158 node $current_node is in the frozen exclusion set" >&2
        exit 3
        ;;
esac
...
for _ in {1..3600}; do
    ...
    if ! kill -0 "$qwen_pid" 2>/dev/null; then
        wait "$qwen_pid" || true
        echo "C158 Qwen worker exited before readiness" >&2
        exit 1
    fi
    sleep 0.05
done
[[ -S "$qwen_socket" ]] || { echo "C158 Qwen readiness timeout" >&2; exit 1; }
...
while IFS=$'\t' read -r home request source receiver_seed channel_seed_base snr \
    ...
    shifter_all env CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=50 CUDA_MPS_CLIENT_PRIORITY=0 \
        python3 /softwall/actual_nrx_repeated_owner.py \
...
shifter_all env CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=80 CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/actual_nrx_repeated_worker.py \
...
shifter_all env CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=80 CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/actual_nrx_repeated_coordinator.py \
    --peer-spec "$peer_spec" --ipc-dir "$state" \
...
pids=""
[[ "$failure" -eq 0 ]] || {
    echo "C158 repeated process failure; artifacts preserved" >&2
    exit 1
}

python3.11 "$SOFTWALL_SCRIPTS/analyze_confirm158_repeated.py" \
...
cleanup
trap - EXIT INT TERM
echo "C158 repeated qualification complete: $result"
