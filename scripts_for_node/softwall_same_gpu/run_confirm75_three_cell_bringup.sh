...
# Confirm75: three radio cells on two NeuralRx endpoints and one GPU.
...
require_allocation
payload_seed=${SOFTWALL_PAYLOAD_SEED:-20750001}
channel_seed_base=${SOFTWALL_CHANNEL_SEED_BASE:-20751000}
iterations=${SOFTWALL_GATE_ITERATIONS:-200}
failure_every=${SOFTWALL_CORRELATED_FAILURE_EVERY:-5}
prefix=${SOFTWALL_GATE_PREFIX:-confirm75_threecell_seed${channel_seed_base}_job${SLURM_JOB_ID}}
...
worker1_pid=""
background_pid=""
tag="tc_${SLURM_JOB_ID}_${BASHPID}"
...
if x['iterations'] != 200 or x['deadline_misses']:
    raise SystemExit('confirm75 operational requalification failed')
print('confirm75 operational requalification passed', flush=True)
PY
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/three_cell_atomic_lease_controller.py \
    --tag-prefix "$tag" --ipc-dir "$state" \
    --engine /softwall_runtime/engines/neural_rx_fp16_full.trt \
    --socket "$background_socket" \
    --output "$raw/${prefix}_controller.json" \
    --iterations "$iterations" --cells 3 --warmup 20 \
    --period-ms 150 --deadline-ms 130 \
...
cleanup
trap - EXIT INT TERM
echo "confirm75 three-cell bring-up completed"
