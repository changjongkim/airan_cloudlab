...
"""Freeze one C159-Q2 variable-context P180 qualification campaign."""
...
SCRIPT_SOURCES = (
    ...
    "c159_q2_coordinator.py",
    "actual_nrx_repeated_control.py",
    "build_confirm159_q2_protocol.py",
    "analyze_confirm159_q2.py",
    "run_confirm159_q2.sh",
...
def main() -> None:
    ...
    parser.add_argument("--minimum-executed-per-class", type=int, default=50)
    ...
    if (args.iterations <= 0 or args.iterations % len(CONTEXT_LENGTHS) != 0
            or args.minimum_executed_per_class <= 0):
        parser.error("iterations must be a positive multiple of six and minimum positive")
    ...
    protocol = {
        "schema": "softwall-confirm159-q2-protocol-v1",
        ...
        "ai_offer": {
            ...
            "minimum_executed_per_class": args.minimum_executed_per_class,
            "admission": (
        ...
        "gates": {
            ...
            "variable_context": (
                "each class follows the frozen round-robin offer, every physical "
                "Qwen completion stays within its class bound, and each class "
                "reaches the frozen minimum executed sample"
            ),
            "launch_revalidation": (
        ...
        "scope": (
            "C159-Q2 variable-context P180 qualification, excluded from "
            "performance confidence intervals. Passing two nodes qualifies only "
            "the six frozen class bounds in this warm synthetic placement; online "
            "BurstGPT performance, WCET, cold/long-idle, production d_MAC and "
            "integrated faults remain UQ."
        ),
        "prespec": {
