...
# V14 physical mechanism arm: prepare/complete are deferred off the RAN path;
...
label=${SOFTWALL_SHARD_LABEL:-v14_ai45_pipelined_job${SLURM_JOB_ID}}
...
value={"schema":"softwall-v14-ai45-pipelined-arm-protocol-v1","status":"frozen-before-run","label":label,"iterations":iterations,"warmup":warmup,"global_policy":policy,"payload_seeds":payload_seeds,"channel_seeds":channel_seeds,"trace":"/softwall_data/softwall_context64_mechanism_trace_v1.json","trace_sha256":"b14388c7356878458031d4e881a0510f42f1b4414493f876cd13d2b881979277","placement":{"gpu0":"four-cell home + two local NRx + Qwen","gpu1":"four-cell home + two local NRx + Qwen"},"shared_resources":"one serializable synthetic context64 request queue; recovery, NRx endpoint/IPC, Qwen execution and physical credits remain per-home","transaction_order":"asynchronous global prepare -> current-horizon revalidation -> local atomic recovery replan+AI lease -> synchronous global commit ACK -> physical AI -> local fence/retire -> asynchronous global complete","cuda_module_loading":"LAZY","period_ms":180,"deadline_ms":155,"nrx_bound_ms":45,"conv_bound_ms":conv_bound_ms,"guard_ms":2,"global_broker_rpc_timeout_ms":5,"global_broker_rpc_admission_bound_ms":7,"global_broker_transaction_budget_ms":7,"admission_ai_guard_ms":9,"target_ai_bound_ms":45,"effective_transaction_bound_ms":54,"endpoint_ring_depth":1,"rejected_recovery_before_exchange":False,"fault_every":5,"rpc_telemetry_required":True,"source_sha256":{key:hashlib.sha256(path.read_bytes()).hexdigest() for key,path in zip(keys,paths)}}
...
for home in 0 1; do
    ...
    shifter_home_aerial "$home" env CUDA_MPS_CLIENT_PRIORITY=0 \
        python3 /softwall/global_trace_pipelined_control_controller.py \
