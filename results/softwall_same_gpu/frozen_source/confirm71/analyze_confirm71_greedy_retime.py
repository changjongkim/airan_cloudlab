...
"""Frozen paired audit of the AI-aware greedy recovery baseline canary."""
...
def main() -> None:
    ...
    protocol = read(base / "confirm71_greedy_retime_protocol.json")
    gates = {
        "source_hashes": all(
    ...
    for entry in protocol["arms"]:
        ...
        if mode == "on":
            gates[f"{prefix}_greedy_path"] = (
                ran["recovery_retime_count"] >= protocol["min_retime_on"]
                and ran["ai_before_recovery_units"] >= protocol["min_before_recovery_ai_on"]
                and ran["runtime_metrics"].get("lease_admitted", 0)
                == ran["ai_before_recovery_units"]
            )
        else:
            gates[f"{prefix}_greedy_disabled"] = (
                ran["recovery_retime_count"] == 0
                and ran["ai_before_recovery_units"] == 0
            )
    pairs = []
    for pair in protocol["pairs"]:
        off, on = arms[pair["off_prefix"]], arms[pair["on_prefix"]]
        ...
        pairs.append({
            "name": pair["name"],
            "off_correct": off["correct_cells"], "on_correct": on["correct_cells"],
            "off_ai_units": off["background_units"], "on_ai_units": on["background_units"],
            "off_timely_ai": off["ai_timely_units"], "on_timely_ai": on["ai_timely_units"],
            ...
            "off_commit_p99_ms": off["commit_response_ms"]["p99"],
            "on_commit_p99_ms": on["commit_response_ms"]["p99"],
        })
    report = {
        "schema": "softwall-confirm71-greedy-recovery-canary-v1",
        "job": protocol["job"], "gates": gates,
        "all_pass": all(gates.values()), "pairs": pairs,
        "interpretation": "Greedy AI-aware recovery retime and one physical AI unit before fallback after NRx observation. Same async pre-NRx AI in both arms. Synthetic P150/D130, AI deadline 50 ms; not atomic recovery+AI transaction, online joint policy, WCET, fault continuation, or production MAC expiry. AI throughput gain is recorded without a frozen gain gate.",
    }
    path = base / f"confirm71_greedy_retime_job{protocol['job']}.json"
