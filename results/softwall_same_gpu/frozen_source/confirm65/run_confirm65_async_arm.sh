...
# Confirm65: asynchronous versus synchronous pre-radio AI RPC arm.
...
require_allocation
mode=${1:?mode must be sync_one or async_one}
case "$mode" in sync_one|async_one) ;; *) echo "invalid mode: $mode" >&2; exit 2 ;; esac
payload_seed=20630003
channel_seed_base=20631000
iterations=${SOFTWALL_GATE_ITERATIONS:-100}
prefix=${SOFTWALL_GATE_PREFIX:-confirm65_${mode}_job${SLURM_JOB_ID}}
