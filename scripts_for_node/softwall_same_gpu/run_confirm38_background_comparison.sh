...
requal="$raw/confirm38_requalification_job${SLURM_JOB_ID}.json"
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    ...
    --seed 20321938 --snr-db -8.5 --channel-seed-base 20329900
python3 - "$requal" <<'PY'
import json, sys
d = json.load(open(sys.argv[1]))
if len(d['records']) != 200 or d['deadline_misses']:
    raise SystemExit('confirm38 requalification failed')
print('confirm38 operational requalification passed')
PY
sleep 20
...
ENDPOINT_TIMEOUT_MS=30 WARMUP=20 SEED=20321039 SNR_DB=-8.5 \
CHANNEL_SEED_BASE=20329000 \
    bash "$SOFTWALL_SCRIPTS/run_same_request_ipc.sh" \
    confirm38_external_background_main 10000
sleep 20

# Round 1 uses the same payload/channel seeds as the external endpoint.
POLICIES=s2_reserved,eager_dual CELLS=1 PERIOD_MS=90 DEADLINE_MS=80 \
NRX_BOUND_MS=20 CONV_BOUND_MS=25 COMMIT_GUARD_MS=2 FAILURE_EVERY=0 \
SNR_DB=-8.5 SEED_BASE=20321038 CHANNEL_SEED_BASE=20329000 \
...
AI_GUARD_MS=2 AI_RPC_TIMEOUT_MS=45 QUIET_SECONDS=20 \
    ...
    1 10000 confirm38_local_background_baselines

shifter_gpu python3 /softwall/analyze_external_background_comparison.py \
    --raw "$raw" --job "$SLURM_JOB_ID" \
    --protocol "$result/confirm38_background_comparison_protocol.json" \
    --output "$result/confirm38_external_background_comparison.md"

cleanup
trap - EXIT INT TERM
echo 'confirm38 background comparison completed'
