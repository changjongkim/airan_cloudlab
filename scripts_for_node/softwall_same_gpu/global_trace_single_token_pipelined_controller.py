#!/usr/bin/env python3
"""V15 controller: V14 pipelining plus one-unlaunched-token ownership."""
...
def pipelined_composed_ai_guard_ns(base_guard_ns, broker_rpc_timeout_ms,
    ...
    broker_budget_ns = round(RPC_BOUND_MS * 1e6) if global_broker_enabled else 0
...
def pipelined_global_queue_status(queue):
    status = {
        ...
        "control_plane": "single-token-pipelined-prepare-and-complete",
        "ownership_contract": getattr(queue, "ownership_contract", None),
