...
# C164 first-request-after-30s-idle physical boundary campaign.
...
label=${SOFTWALL_C164_LABEL:-c164_idle30_job${SLURM_JOB_ID}}
campaign=${SOFTWALL_C164_CAMPAIGN:-development}
seed_base=${SOFTWALL_C164_SEED_BASE:-45000000}
excluded_nodes=${SOFTWALL_C164_EXCLUDED_NODES:-nid002288,nid002100,nid001109,nid001085,nid001064,nid001124,nid001177,nid001308,nid001204,nid001632,nid001824,nid001025,nid001145,nid001069,nid001044,nid001265,nid001353}
iterations=${SOFTWALL_C164_ITERATIONS:-60}
period_ms=${SOFTWALL_C164_PERIOD_MS:-180}
warmup=${SOFTWALL_C164_WARMUP:-10}
required_idle_s=${SOFTWALL_C164_REQUIRED_IDLE_S:-30}
release_lead_ms=${SOFTWALL_C164_RELEASE_LEAD_MS:-3000}
reverse_cases=${SOFTWALL_C164_REVERSE_CASES:-0}
...
idle_marker="$raw/${label}_idle_marker.json"
...
rm -f "$qwen_socket" "$qwen_output" "$nrx_output" \
    "$coordinator_output" "$idle_marker" "$base_result" "$result"
...
python3.11 "$SOFTWALL_SCRIPTS/build_c164_idle_protocol.py" \
    --protocol "$protocol" --scripts-root "$SOFTWALL_SCRIPTS" \
    --required-idle-s "$required_idle_s" \
    --release-lead-ms "$release_lead_ms"

current_node=$(hostname)
case ",$excluded_nodes," in
    *",$current_node,"*)
        echo "C164 node $current_node is in the frozen exclusion set" >&2
        exit 3
        ;;
esac
...
shifter_all env CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=80 CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/c164_idle_coordinator.py \
    --peer-spec "$peer_spec" --ipc-dir "$state" \
    ...
    --pre-schedule-idle-s "$required_idle_s" \
    --lifecycle-marker "$idle_marker" \
    "${reverse_arg[@]}" \
...
python3.11 "$SOFTWALL_SCRIPTS/analyze_c162_boundary.py" \
    ...
    --scripts-root "$SOFTWALL_SCRIPTS" --task1-root "$SOFTWALL_TASK1" \
    --output "$base_result"
python3.11 "$SOFTWALL_SCRIPTS/analyze_c164_idle.py" \
    --protocol "$protocol" --base-result "$base_result" \
    --marker "$idle_marker" --coordinator "$coordinator_output" \
    --nrx-worker "$nrx_output" --qwen "$qwen_output" \
    --lifecycle-model "$result_root/c164_lifecycle_model_v1.json" \
...
cleanup
trap - EXIT INT TERM
echo "C164 idle30 physical boundary campaign complete: $result"
