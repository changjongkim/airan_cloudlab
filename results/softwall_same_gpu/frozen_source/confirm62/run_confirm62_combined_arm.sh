...
# Confirm62: combined low-feature, queue-aware, early mandatory baseline arm.
...
require_allocation
mode=${1:?mode must be always or low_threshold}
case "$mode" in always|low_threshold) ;; *) echo "invalid mode: $mode" >&2; exit 2 ;; esac
payload_seed=20590001
channel_seed_base=20591000
iterations=${SOFTWALL_GATE_ITERATIONS:-200}
prefix=${SOFTWALL_GATE_PREFIX:-confirm62_${mode}_job${SLURM_JOB_ID}}
...
if x['iterations'] != 200 or x['deadline_misses']:
    raise SystemExit('confirm62 operational requalification failed')
print('confirm62 operational requalification passed', flush=True)
PY
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/two_endpoint_combined_baseline_controller.py \
...
cleanup
trap - EXIT INT TERM
echo "confirm62 mode=$mode completed"
