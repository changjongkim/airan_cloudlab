...
# C159-Q1 pre-staged P180 actual TensorRT NeuralRx qualification.
...
label=${SOFTWALL_C159_LABEL:-confirm159q1_prestaged_job${SLURM_JOB_ID}}
campaign=${SOFTWALL_C159_CAMPAIGN:-development}
seed_base=${SOFTWALL_C159_SEED_BASE:-32000000}
excluded_nodes=${SOFTWALL_C159_EXCLUDED_NODES:-nid002288,nid002100,nid001109,nid001085,nid001064}
iterations=${SOFTWALL_C159_ITERATIONS:-250}
period_ms=${SOFTWALL_C159_PERIOD_MS:-180}
warmup=${SOFTWALL_C159_WARMUP:-10}
release_lead_ms=${SOFTWALL_C159_RELEASE_LEAD_MS:-3000}
...
qwen_socket="$SOFTWALL_ROOT/mps/$SLURM_JOB_ID/c159.sock"
...
python3.11 "$SOFTWALL_SCRIPTS/build_confirm159_q1_protocol.py" \
...
case ",$excluded_nodes," in
    *",$current_node,"*)
        echo "C159-Q1 node $current_node is in the frozen exclusion set" >&2
        exit 3
        ;;
esac
...
for _ in {1..3600}; do
    ...
    if ! kill -0 "$qwen_pid" 2>/dev/null; then
        wait "$qwen_pid" || true
        echo "C159-Q1 Qwen worker exited before readiness" >&2
        exit 1
    fi
    sleep 0.05
done
[[ -S "$qwen_socket" ]] || { echo "C159-Q1 Qwen readiness timeout" >&2; exit 1; }
...
shifter_all env CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=80 CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/c159_q1_coordinator.py \
    --peer-spec "$peer_spec" --ipc-dir "$state" \
...
pids=""
[[ "$failure" -eq 0 ]] || {
    echo "C159-Q1 process failure; artifacts preserved" >&2
    exit 1
}

python3.11 "$SOFTWALL_SCRIPTS/analyze_confirm159_q1.py" \
...
cleanup
trap - EXIT INT TERM
echo "C159-Q1 P180 qualification complete: $result"
