...
"""Freeze one C161 phase-1 physical fault campaign."""
...
SCRIPT_SOURCES = (
    ...
    "c161_phase1_coordinator.py",
    "actual_nrx_repeated_control.py",
    "build_c161_phase1_protocol.py",
    "analyze_c161_phase1.py",
    "run_c161_phase1.sh",
    "test_c159_q2_variable_plan.py",
    "test_bounded_launch_revalidation.py",
    "test_actual_nrx_batch_recovery_plan_v1.py",
    "test_c161_phase1_faults.py",
...
def main() -> None:
    ...
    parser.add_argument("--arm-length", type=int, default=30)
    parser.add_argument("--fault-hold-ms", type=float, default=10.0)
    ...
    arms = tuple(value for value in args.arm_order.split(",") if value)
    supported_arms = {
        "no_fault", "correlated_all_fail", "latest_start_nonlaunch"
    }
    if (args.iterations <= 0 or args.iterations % len(CONTEXT_LENGTHS) != 0
            or args.arm_length <= 0 or args.fault_hold_ms <= 0
            or set(arms) != supported_arms or len(arms) != 3
            or args.iterations != args.arm_length * len(arms)):
        parser.error("invalid C161 phase-1 arm/iteration configuration")
    ...
    if prespec.get("dataset", {}).get("holdout_materialized") is not False:
        parser.error("C159-Q2 requires an unopened holdout")
    ...
    protocol = {
        "schema": "softwall-c161-phase1-protocol-v1",
        ...
        "fault_plan": {
            ...
            "fault_hold_ms": args.fault_hold_ms,
            "semantics": {
                "no_fault": "actual timely NeuralRx outcomes drive the batch transition",
                "correlated_all_fail": (
                    "all physically observed successes are suppressed before the "
                    "batch transition; four accepted debts execute recovery"
                ),
                "latest_start_nonlaunch": (
                    "after a committed AI lease, host dispatch is held past the "
                    "worker latest-start; no Qwen kernel may launch"
                ),
            },
        },
        "mode": {
            ...
            "warmup": args.warmup,
            "qwen_context_lengths": CONTEXT_LENGTHS,
        ...
        "ai_offer": {
            "order": CONTEXT_LENGTHS,
            "rule": "deterministic round-robin by one-based radio epoch",
            "offered_per_class": args.iterations // len(CONTEXT_LENGTHS),
            "admission": (
                "launch-time global certificate with the class-specific bound; "
                "certificate rejection is an envelope observation, not a failure"
            ),
        },
        ...
        "gates": {
            ...
            "phase1_fault_coverage": (
                "each arm has the frozen length; correlated epochs execute four "
                "recoveries; every accepted latest-start fault produces a guarded "
                "non-launch and an empty-lease retire"
            ),
            "launch_revalidation": (
                "a certificate build above 5 ms is discarded and rebuilt from "
                "the current clock, for at most three attempts"
            ),
            "physical_start_guard": (
                "the Qwen worker starts no kernel after the committed lease's "
                "absolute latest-start time; a guarded non-launch safely retires "
                "the empty lease and is reported separately"
            ),
            "transition": (
                "physical commit agrees with effective outcome; correlated arm "
                "retains debt even when the physical NeuralRx result succeeded"
            ),
            "atomic_outcome_batch": (
                "all successes observed at the common cutoff are removed in one "
                "transaction before the current-time recovery/AI certificate; "
                "no sequential partial-outcome state is exposed"
            ),
        ...
        "scope": (
            "C161 phase-1 physical fault semantics in the C159-Q2 warm P180 "
            "placement. It covers correlated outcome suppression and an enforced "
            "physical latest-start non-launch; response loss, worker crash, GPU "
            "hang, production d_MAC and cross-family behavior remain UQ."
        ),
        "prespec": {
