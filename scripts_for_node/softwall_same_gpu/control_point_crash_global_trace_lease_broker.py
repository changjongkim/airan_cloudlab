#!/usr/bin/env python3
"""Global lease broker with a deterministic post-apply control fault point."""
...
class BrokerServer(socketserver.ThreadingUnixStreamServer):
    ...
    def __init__(self, path: str, state: GlobalTraceLeaseState,
                 ...
                 crash_after_home: int | None = None,
                 crash_after_number: int | None = None):
        self.state = state
    ...
    def should_crash_after_operation(
            self, operation: str, request: dict, result: dict | None) -> bool:
        """Apply one state-changing operation, then exit before its reply.

        The state transition precedes this hook, so the client cannot know
        whether prepare/commit/complete succeeded.  A prepare returning None
        does not hold a request and therefore is not counted as an applied
        state transition.
        """
        if operation == "prepare" and result is None:
            return False
        home = request["home"]
        with self.fault_lock:
            key = f"{operation}:h{home}"
            count = self.operation_counts.get(key, 0) + 1
            self.operation_counts[key] = count
            if (operation != self.crash_after_operation
                    or home != self.crash_after_home
                    or count != self.crash_after_number):
                return False
            self.faults.append({
                "fault": f"broker_exit_after_{operation}_apply",
                "operation": operation,
                "home": home,
                "operation_number": count,
                "token": request.get("token", result.get("broker_token") if result else None),
        ...
        while True:
            ...
            try:
                ...
                if op == "init":
                    ...
                    if self.server.should_crash_after_operation(op, request, result):
                        os._exit(86)
                elif op == "commit":
...
def main() -> None:
    ...
    parser.add_argument("--crash-after-operation", choices=("prepare", "commit", "complete"))
    ...
    configured = (
        args.crash_after_operation, args.crash_after_home, args.crash_after_number
    )
