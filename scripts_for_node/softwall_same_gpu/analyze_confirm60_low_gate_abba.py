...
"""Frozen paired audit for the two-cell low-feature skip ingredient."""
...
def main() -> None:
    ...
    raw = result / "raw"
    protocol = read(result / "confirm60_low_gate_abba_protocol.json")
    ...
    for pair in (1, 2):
        for mode in ("always", "low_threshold"):
            ...
            prefix = f"confirm60_{label}_job{args.job}"
            ...
            gates[label + "_features_online"] = all(
                ...
                and row["gate_skipped"] == (
                    mode == "low_threshold" and row["channel_estimate_power"] < threshold
                )
    ...
    for pair in (1, 2):
        ...
        gated, _, gate_rows, _ = arms[(pair, "low_threshold")]
        same_trace = all(
            a["index"] == b["index"]
            and a["cell"] == b["cell"]
        ...
        ci = paired_cluster_interval(always_rows, gate_rows, 20600000 + pair)
    ...
    report = {
        "schema": "softwall-confirm60-low-gate-abba-v1",
        "job": args.job,
        "execution_order": protocol["execution_order"],
        "comparisons": comparisons,
        "gates": gates,
        "all_pass": all(gates.values()),
        "interpretation": "Low-feature failure-aware gate with pinned endpoints and AI after radio; not the full strong combined baseline or joint policy. P150/D130 is synthetic.",
    }
    output = result / f"confirm60_low_gate_abba_job{args.job}.json"
