...
# post-apply prepare/commit/complete broker crash and fail-closed RAN containment.
...
label=${SOFTWALL_SHARD_LABEL:-global_ai_control_point_broker_crash_job${SLURM_JOB_ID}}
...
out=Path(sys.argv[1]);iterations=int(sys.argv[2]);warmup=int(sys.argv[3]);label=sys.argv[4];policy=sys.argv[5];conv_bound_ms=float(sys.argv[6]);operation=sys.argv[7];number=int(sys.argv[8])
keys=["/softwall/global_trace_fully_budgeted_fault_contained_controller.py","/softwall/deadline_fault_contained_global_trace_client.py","/softwall/control_point_crash_global_trace_lease_broker.py","/softwall/same_request_ipc_worker.py","/softwall/trace_qwen_worker.py","/softwall_task1/isca_v2/dart_runtime.py","/softwall_task1/isca_v2/cuda_ipc_channel.py"]
paths=[Path(value) for value in sys.argv[9:]]
value={"schema":"softwall-global-ai-control-point-broker-crash-arm-protocol-v1","status":"frozen-before-run","label":label,"iterations":iterations,"warmup":warmup,"global_policy":policy,"placement":{"gpu0":"four-cell home + two local NRx + Qwen","gpu1":"four-cell home + two local NRx + Qwen"},"shared_resources":"one serializable BurstGPT request queue; recovery, NRx endpoint/IPC, Qwen execution and physical credits remain per-home","transaction_order":"observe NRx -> certificate order -> bounded global prepare/hold -> local atomic replan+AI lease -> bounded global commit -> physical AI -> bounded global complete; broker fail-stop disables global AI clients without retry; local RAN certificates continue","fault_injection":{"home":0,"operation":operation,"operation_number":number,"point":"process calls os._exit(86) immediately after the configured state transition, before reply; no synchronous fault-path logging","expected_exit_code":86,"broker_rpc_timeout_ms":5},"cuda_module_loading":"LAZY","period_ms":180,"deadline_ms":155,"nrx_bound_ms":45,"conv_bound_ms":conv_bound_ms,"guard_ms":2,"global_broker_rpc_timeout_ms":5,"global_broker_transaction_budget_ms":15,"admission_ai_guard_ms":17,"fault_every":5,"source_sha256":{key:hashlib.sha256(path.read_bytes()).hexdigest() for key,path in zip(keys,paths)}}
...
value={'schema':'softwall-global-ai-control-point-broker-crash-arm-result-v1','protocol':str(protocol),'crash_operation':operation,'broker_exit_status':broker_status,'fault_detected_ns':fault_detected_ns,'target_request_id':target_request_id,'operational_global_queue_faults':operational_faults,'radio_records_after_fault_detection':radio_after,'homes':homes,'gates':gates,'all_pass':all(gates.values()),'artifact_sha256':{str(path):hashlib.sha256(path.read_bytes()).hexdigest() for path in artifacts},'scope':f'5 ms per-RPC and 15 ms fully admission-budgeted broker fail-stop after applying home0 {operation}30; no synchronous fault-path logging; all global AI clients fail closed while local RAN certificates continue'}
tmp=out.with_suffix('.tmp');tmp.write_text(json.dumps(value,indent=2));tmp.replace(out);print(json.dumps(gates,indent=2))
if not value['all_pass']:raise SystemExit('control-point broker crash-containment arm failed')
PY
cleanup
trap - EXIT INT TERM
echo "global AI control-point broker crash arm complete: $result"
