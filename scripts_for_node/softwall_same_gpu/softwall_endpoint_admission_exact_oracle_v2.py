#!/usr/bin/env python3
"""Independent exact oracle for endpoint admission with physical ring limits.
...
def exact_maximum_admission(jobs, endpoint_bounds_ms,
    ...
    if endpoint_ring_depths is None:
        endpoint_ring_depths = [len(jobs) for _ in bounds]
    depths = tuple(int(value) for value in endpoint_ring_depths)
    if len(depths) != len(bounds) or any(value <= 0 for value in depths):
        raise ValueError("endpoint ring depth vector is invalid")
    if not bounds:
    ...
    def solve(index, counts):
        if index == len(jobs):
            return 0
        best = solve(index + 1, counts)
        best_choice = None
        candidates = []
        for endpoint, bound in enumerate(bounds):
            if counts[endpoint] >= depths[endpoint]:
                continue
            finish = round((counts[endpoint] + 1) * bound, 9)
            if finish <= jobs[index]["cutoff_ms"] + EPSILON:
                next_counts = list(counts)
                next_counts[endpoint] += 1
                admitted = 1 + solve(index + 1, tuple(next_counts))
                candidates.append((
                    admitted, finish, endpoint, tuple(next_counts)
                ))
        if candidates:
        ...
        choices[(index, counts)] = best_choice
        return best

    initial = tuple(0 for _ in bounds)
    admitted = solve(0, initial)
    assignment = []
    counts = initial
    for index in range(len(jobs)):
        choice = choices[(index, counts)]
        if choice is None:
            assignment.append(None)
        else:
            endpoint, counts = choice
...
def verify_prediction_schedule(result, scenario):
    ...
    if "home_endpoint_ring_depths" in scenario:
        for home, depths in enumerate(scenario["home_endpoint_ring_depths"]):
            for endpoint, depth in enumerate(depths):
                name = "h{}:e{}".format(home, endpoint)
                if endpoint_count.get(name, 0) > int(depth):
                    errors.append("endpoint {} exceeds ring depth".format(name))
    elif "endpoint_ring_depths" in scenario:
        for endpoint, depth in enumerate(scenario["endpoint_ring_depths"]):
            name = str(endpoint)
            if endpoint_count.get(name, 0) > int(depth):
                errors.append("endpoint {} exceeds ring depth".format(name))
    rejected_first = scenario.get("rejected_recovery_before_exchange", True)
    if isinstance(rejected_first, bool):
        rejected_first = [rejected_first for _ in admitted_by_home]
    expected_delta = []
    for count, cells, runs_first in zip(
            admitted_by_home, scenario["home_cell_counts"], rejected_first):
        deletions = (
            max(0, count - 1)
            if runs_first else min(count, max(0, cells - 1))
        )
        expected_delta.append(deletions * scenario["recovery_bound_ms"])
...
def audit_scenario(scenario, result):
    ...
    if "home_endpoint_bounds_ms" in scenario:
        for home, (cells, bounds) in enumerate(zip(
            ...
            depths = scenario.get("home_endpoint_ring_depths", [
                [cells for _ in bounds]
                for cells, bounds in zip(
                    scenario["home_cell_counts"],
                    scenario["home_endpoint_bounds_ms"],
                )
            ])[home]
            oracle = exact_maximum_admission(jobs, bounds, depths)
        ...
        depths = scenario.get(
            "endpoint_ring_depths", [scenario["cells"] for _ in bounds]
        )
        oracle = exact_maximum_admission(jobs, bounds, depths)
        pool_results.append({
            "home": "shared-global",
        ...
        for deadline in (30, 45, 60, 75, 90, 120, 155):
            for recovery in (5, 10, 12, 20, 25):
                for bounds in endpoint_patterns:
                    for uniform_depth in (1, 2, sum(homes)):
                        depths = tuple(uniform_depth for _ in bounds)
                        jobs = build_jobs(homes, deadline, 2, recovery)
                        oracle = exact_maximum_admission(jobs, bounds, depths)
                        greedy = slot_greedy_count(jobs, bounds, depths)
                        cases += 1
                        ...
                        if greedy != oracle["admitted"]:
                            gaps.append({
                                ...
                                "endpoint_ring_depths": list(depths),
                                "greedy": greedy,
...
def main():
    ...
    output = {
        "schema": "softwall-endpoint-admission-exact-oracle-audit-v2",
        ...
        "scope": (
            ...
            "service-bound model with finite endpoint ring credits; not a "
            "queueing or WCET proof."
        ),
