...
# Confirm105: within-process matched one-versus-two NeuralRx count probe.
...
prefix=${SOFTWALL_GATE_PREFIX:-confirm105_interleaved_seed${channel_seed_base}_job${SLURM_JOB_ID}}
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/four_cell_nrx_count_interleaved_controller.py \
    ...
    --max-nrx-admissions 2 --nrx-count-pattern 1221 \
...
cleanup
trap - EXIT INT TERM
echo "confirm105 interleaved NeuralRx-count Qwen run completed"
