            ...
            try:
                ...
                if op == "init":
                    ...
                    if self.server.should_crash_after_commit(request, result):
                        if self.server.crash_marker is None:
                            raise RuntimeError("broker crash marker is required")
                        atomic_json(self.server.crash_marker, {
                            "schema": "softwall-global-broker-crash-marker-v1",
                            "fault": self.server.faults[-1],
                            "summary_before_exit": self.server.state.snapshot(),
                            "exit_code": 86,
                        })
                        os._exit(86)
                elif op == "abort":
...
def main() -> None:
    ...
    if args.crash_after_commit_home is not None and args.crash_marker is None:
        parser.error("broker-crash fault requires --crash-marker")
    server = BrokerServer(
        str(socket_path), state,
        args.crash_after_commit_home, args.crash_after_commit_number,
        None if args.crash_marker is None else Path(args.crash_marker),
    )
    try:
