...
# Four disjoint three-cell RAN homes with synchronized releases on GPU0--GPU3.
...
label=${SOFTWALL_SHARD_LABEL:-sharded_four_home_job${SLURM_JOB_ID}}
payload_seed_base=${SOFTWALL_SHARD_PAYLOAD_SEED_BASE:-25000051}
channel_seed_base=${SOFTWALL_SHARD_CHANNEL_SEED_BASE:-25001000}
homes=4
cells=3
...
for home in 0 1 2 3; do
    mkdir -p "$state_root/h${home}"
...
python3 - "$protocol" "$iterations" "$warmup" "$label" "$payload_seed_base" "$channel_seed_base" \
    "$SOFTWALL_SCRIPTS/sharded_trace_baseline_controller.py" \
...
payload_seed_base=int(sys.argv[5]);channel_seed_base=int(sys.argv[6])
keys=["/softwall/sharded_trace_baseline_controller.py","/softwall/same_request_ipc_worker.py","/softwall/trace_qwen_worker.py","/softwall_task1/isca_v2/dart_runtime.py","/softwall_task1/isca_v2/cuda_ipc_channel.py"]
paths=[Path(value) for value in sys.argv[7:]]
value={"schema":"softwall-sharded-four-home-arm-protocol-v1","status":"frozen-before-run","label":label,"iterations":iterations,"warmup":warmup,"homes":4,"cells_per_home":3,"payload_seed_base":payload_seed_base,"channel_seed_base":channel_seed_base,"placement":{f"gpu{home}":"three-cell home + two local NRx + Qwen" for home in range(4)},"shared_resources":"MPS control only; recovery, endpoint, IPC, Qwen and trace queues are disjoint by home","cuda_module_loading":"LAZY","period_ms":180,"deadline_ms":155,"nrx_bound_ms":45,"conv_bound_ms":12,"guard_ms":2,"fault_every":5,"source_sha256":{key:hashlib.sha256(path.read_bytes()).hexdigest() for key,path in zip(keys,paths)}}
...
cleanup() {
    ...
    for home in 0 1 2 3; do
        rm -f "$SOFTWALL_ROOT/mps/$SLURM_JOB_ID/sh${home}_${BASHPID}.sock"
    done
...
for home in 0 1 2 3; do
    socket="$SOFTWALL_ROOT/mps/$SLURM_JOB_ID/sh${home}_${BASHPID}.sock"
    payload=$((payload_seed_base + home * 10000))
    channel=$((channel_seed_base + home * 10000))
    shifter_home_aerial "$home" env CUDA_MPS_CLIENT_PRIORITY=0 \
        python3 /softwall/sharded_trace_baseline_controller.py \
        ...
        --cells "$cells" --nrx-endpoints 2 --warmup "$warmup" --period-ms 180 --deadline-ms 155 \
        ...
        --start-barrier-dir "$barrier" --start-barrier-member "$home" --start-barrier-participants "$homes" &
    pids="$pids $!"
...
python3 - "$protocol" "$raw" "$label" "$warmup" "$result" <<'PY'
import hashlib,json,sys
from collections import Counter
from pathlib import Path
protocol,raw,label,warmup,out=Path(sys.argv[1]),Path(sys.argv[2]),sys.argv[3],int(sys.argv[4]),Path(sys.argv[5])
p=json.loads(protocol.read_text());homes_count=p['homes'];cells=p['cells_per_home']
homes=[];artifacts=[protocol]
for home in range(homes_count):
 ...
 for endpoint in (0,1):
  ...
  warmup_cells=sum(cell % 2 == endpoint for cell in range(cells))
  expected=admissions[f'nrx{endpoint}']+(warmup+1)*warmup_cells
...
gates={'four_homes':len(homes)==homes_count,'same_first_release':len({h['first_release_ns'] for h in homes})==1,'twelve_cells':sum(x['summary']['records'] for x in homes)==homes_count*cells*p['iterations'],'worker_count_match':all(w['count_match'] for h in homes for w in h['workers']),'system_safety':all(all(h['safety'].values()) for h in homes)}
value={'schema':'softwall-sharded-four-home-arm-result-v1','protocol':str(protocol),'homes':homes,'gates':gates,'all_pass':all(gates.values()),'artifact_sha256':{str(path):hashlib.sha256(path.read_bytes()).hexdigest() for path in artifacts},'scope':'disjoint-product four-home certificate gate; no shared endpoint or global AI lease'}
tmp=out.with_suffix('.tmp');tmp.write_text(json.dumps(value,indent=2));tmp.replace(out);print(json.dumps(gates,indent=2))
if not value['all_pass']:raise SystemExit('sharded four-home arm failed')
PY

cleanup
trap - EXIT INT TERM
echo "sharded four-home arm complete: $result"
