...
- **A. `PuschRxPipelineFactory` (pyaerial high-level batch API)** —
  attempted to instantiate the segfaulting factory that
  `real_l1.py`'s original comment mentioned. Result: **confirmed segfault
  (exit code 139)** on this build. pyaerial 25.3.2 does not expose a
  working batched PUSCH RX pipeline; only the per-component API works.
  Would require pyaerial C++ binding fix (out of scope).
