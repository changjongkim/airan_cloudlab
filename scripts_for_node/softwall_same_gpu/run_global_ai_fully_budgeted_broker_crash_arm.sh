...
label=${SOFTWALL_SHARD_LABEL:-global_ai_fully_budgeted_broker_crash_job${SLURM_JOB_ID}}
...
value={"schema":"softwall-global-ai-fully-budgeted-broker-crash-arm-protocol-v1","status":"frozen-before-run","label":label,"iterations":iterations,"warmup":warmup,"global_policy":policy,"placement":{"gpu0":"four-cell home + two local NRx + Qwen","gpu1":"four-cell home + two local NRx + Qwen"},"shared_resources":"one serializable BurstGPT request queue; recovery, NRx endpoint/IPC, Qwen execution and physical credits remain per-home","transaction_order":"observe NRx -> certificate order -> bounded global prepare/hold -> local atomic replan+AI lease -> bounded global commit -> physical AI -> bounded global complete; broker fail-stop disables global AI clients without retry; local RAN certificates continue","fault_injection":{"home":0,"commit_number":30,"point":"process calls os._exit(86) immediately after broker applies commit, before reply; no synchronous fault-path logging","expected_exit_code":86,"broker_rpc_timeout_ms":5},"cuda_module_loading":"LAZY","period_ms":180,"deadline_ms":155,"nrx_bound_ms":45,"conv_bound_ms":conv_bound_ms,"guard_ms":2,"global_broker_rpc_timeout_ms":5,"global_broker_transaction_budget_ms":15,"admission_ai_guard_ms":17,"fault_every":5,"source_sha256":{key:hashlib.sha256(path.read_bytes()).hexdigest() for key,path in zip(keys,paths)}}
...
gates={
 ...
 'control_budget_accounted':all(c['global_broker_transaction_budget_ms']==15 and c['admission_ai_guard_ms']==17 for c in controllers),
...
cleanup
trap - EXIT INT TERM
echo "global AI fully-budgeted broker crash arm complete: $result"
