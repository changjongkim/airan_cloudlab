#!/usr/bin/env python3
"""Request-owned two-cell PHY with queue-aware cross-endpoint CUDA IPC.

Separate one AI unit before recovery from moving the recovery credit later.
Both arms use the same asynchronous pre-NRx AI unit and the same safety check.

Each PHY request keeps its own receiver state while the endpoint owns only
CUDA IPC buffers. AI still runs after radio; this is transport qualification,
not a full strong-baseline or joint-policy comparison.
"""
...
def main() -> None:
    ...
    parser.add_argument("--ai-aware-recovery", choices=("lease_only", "on"), required=True)
    ...
    try:
        ...
        for index in range(args.iterations):
            ...
            admission_order = [1, 0] if args.alternate_admission_order and index % 2 else [0, 1]
            ...
            if args.ai_aware_recovery in ("lease_only", "on") and pending:
                # A successful other cell has physically retired its credit.
                # Greedily move the sole surviving obligation to its latest
                # conflict-free position before considering AI demand.
                if args.ai_aware_recovery == "on" and len(pending) == 1:
                    tx = pending[0]
                    latest_start_ns = (
                        tx.request.deadline_ns - guard_ns - conv_bound_ns
                    )
                    if latest_start_ns > tx.fallback_reservation.start_ns:
                        moved = runtime.replan_fallbacks(
                            [(tx, latest_start_ns, tx.fallback_reservation.lane)],
                            time.perf_counter_ns(),
                        )
                        recovery_action["retimed"] = moved is not None
                ...
                if (background.enabled and
                    now_ns + ai_budget_ns + ai_guard_ns
                        <= min(earliest_recovery_ns, ai_deadline_ns)):
                    lease = runtime.try_lease(
                        "ai", now_ns, earliest_recovery_ns,
                    ...
                    if lease is not None:
                        ...
                        if not background.enabled or len(background.records) != prior_count + 1:
                            raise RuntimeError("greedy AI physical completion unconfirmed")
                        ai_record = background.records[-1]
                        ...
                        if ai_record["fallback_guard_violation"]:
                            raise RuntimeError("greedy AI crossed recovery guard")
    ...
    result = {
        ...
        "ai_timely_units": sum(
            row["returned_ns"] <= records[2 * row["release_index"]]["release_ns"]
