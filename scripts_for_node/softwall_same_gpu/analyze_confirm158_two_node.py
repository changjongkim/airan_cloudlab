...
"""Combine the frozen C158 development and independent-node holdout runs."""
...
from build_confirm158_repeated_protocol import source_hashes
...
def campaign_rows(peer_spec: dict, coordinator: dict, nrx: dict):
    ...
    return owners, owner_records, recoveries, qwen, nrx["records"]
...
def main() -> None:
    ...
    for label in (args.development_label, args.holdout_label):
        ...
        nrx = load(nrx_path)
        owners, owner_records, recoveries, qwen, nrx_records = campaign_rows(
            spec, coordinator, nrx
        )
        campaigns.append({
            "label": label,
    ...
    gates = {
        ...
        "combined_sample_at_least_1000": len(all_owner) >= 1000,
    ...
    for row in campaigns:
        ...
        artifact_paths.extend(Path(owner["owner_output"])
                              for owner in row["protocol"].get("peers", []))

    value = {
        "schema": "softwall-confirm158-two-node-result-v1",
        "status": (
            "C158_TWO_NODE_REPEATED_ACTUAL_NRX_PASS"
            if all(gates.values()) else "C158_TWO_NODE_REPEATED_ACTUAL_NRX_FAIL"
        ),
        ...
        "claim_scope": (
            ...
            "P600/D155 actual-NeuralRx transition mode. The 600 ms period "
            "provides a declared post-expiry staging interval; the radio "
            "contract remains D155 with NRx45, conventional25, AI35. This is "
            "not WCET, cold/long-idle, production d_MAC, fault, throughput, "
            "or cross-family evidence, and pilot samples are excluded from "
