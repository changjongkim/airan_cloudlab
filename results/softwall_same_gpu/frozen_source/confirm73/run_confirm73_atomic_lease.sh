...
# Confirm73: physical canary for one atomic recovery-replan and AI lease.
...
require_allocation
payload_seed=${SOFTWALL_PAYLOAD_SEED:-20730001}
channel_seed_base=${SOFTWALL_CHANNEL_SEED_BASE:-20731000}
iterations=${SOFTWALL_GATE_ITERATIONS:-200}
prefix=${SOFTWALL_GATE_PREFIX:-confirm73_atomic_seed${channel_seed_base}_job${SLURM_JOB_ID}}
...
worker1_pid=""
background_pid=""
tag="ja_${SLURM_JOB_ID}_${BASHPID}"
...
if x['iterations'] != 200 or x['deadline_misses']:
    raise SystemExit('confirm73 operational requalification failed')
print('confirm73 operational requalification passed', flush=True)
PY
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/two_endpoint_atomic_lease_controller.py \
...
cleanup
trap - EXIT INT TERM
echo "confirm73 atomic lease completed"
