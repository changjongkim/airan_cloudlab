#!/usr/bin/env python3
"""Deterministic envelope checker with decision-time conditional slack.

Version 6 preserves the v5 home-local and control-budget checks, and separates
coarse W+Delta geometry from a sufficient exchange window at the controller's
actual observe-all decision boundary.
"""
...
def decision_time_exchange_windows(admission, home_cell_counts, deadline_ms,
    ...
    """Conservative per-home window at the current observe-all decision point.

    The current controller waits for all admitted NeuralRx replies, executes
    conventional recovery for every rejected NeuralRx job, and only then
    attempts a joint exchange.  The most favorable branch that still has a
    recovery boundary leaves one admitted recovery pending.
    """
    windows = []
    for home, cells in enumerate(home_cell_counts):
        admitted = [
        ...
        rejected_charge = rejected_count * recovery_ms
        decision_time = latest_observation + rejected_charge
        one_pending_start = deadline_ms - guard_ms - recovery_ms
        windows.append({
            "home": home,
            "admitted_nrx": admitted_count,
            "rejected_nrx": rejected_count,
            ...
            "max_safe_exchange_transaction_ms": max(
                0.0, one_pending_start - decision_time
            ),
        })
    return windows
...
def evaluate(scenario):
    ...
    decision_windows = decision_time_exchange_windows(
        admission, home_cells, deadline, guard, recovery
    )
    ai = []
...
def main():
    ...
    output = {
        "schema": "softwall-deterministic-envelope-v6",
