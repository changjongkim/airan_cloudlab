...
# Confirm56: qualify host-to-commit timing with the post-Confirm55 controller.
# This preserves the Confirm55 physical workload and does not test a joint policy.
...
requal="$raw/confirm56_requalification_job${SLURM_JOB_ID}.json"
...
if x['iterations'] != 200 or x['deadline_misses']:
    raise SystemExit('confirm56 operational requalification failed')
print('confirm56 operational requalification passed', flush=True)
PY

sleep 20
for cell in 0 1; do
    output="$raw/confirm56_host_path_job${SLURM_JOB_ID}_worker${cell}.json"
...
shifter_gpu env CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=20 CUDA_MPS_CLIENT_PRIORITY=1 \
    ...
    --output "$raw/confirm56_host_path_job${SLURM_JOB_ID}_background.json" &
background_pid=$!
for _ in {1..1200}; do
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/two_endpoint_bringup_controller_v2.py \
    --tag-prefix "$tag" --ipc-dir "$state" \
    --engine /softwall_runtime/engines/neural_rx_fp16_full.trt \
    --socket "$background_socket" \
    --output "$raw/confirm56_host_path_job${SLURM_JOB_ID}_controller.json" \
...
cleanup
trap - EXIT INT TERM
echo 'confirm56 host-path qualification completed'
