...
# C138 arm: preserve the C134 post-apply broker fail-stop mechanism and add
# attempted-RPC wall-clock telemetry, including the call that detects failure.
...
label=${SOFTWALL_SHARD_LABEL:-service_bound_fault_telemetry_job${SLURM_JOB_ID}}
...
python3 - "$protocol" "$iterations" "$warmup" "$label" "$global_policy" "$conv_bound_ms" "$crash_operation" "$crash_number" \
...
out=Path(sys.argv[1]);iterations=int(sys.argv[2]);warmup=int(sys.argv[3]);label=sys.argv[4];policy=sys.argv[5];conv_bound_ms=float(sys.argv[6]);operation=sys.argv[7];number=int(sys.argv[8]);payload_seeds=[int(sys.argv[9]),int(sys.argv[10])];channel_seeds=[int(sys.argv[11]),int(sys.argv[12])]
keys=["/softwall/global_trace_fully_budgeted_fault_contained_controller.py","/softwall/global_trace_service_bound_fault_controller.py","/softwall/deadline_fault_contained_global_trace_client.py","/softwall/instrumented_deadline_global_trace_client_v2.py","/softwall/control_point_crash_global_trace_lease_broker.py","/softwall/same_request_ipc_worker.py","/softwall/trace_qwen_worker.py","/softwall_task1/isca_v2/dart_runtime.py","/softwall_task1/isca_v2/cuda_ipc_channel.py"]
paths=[Path(value) for value in sys.argv[13:]]
value={"schema":"softwall-service-bound-fault-telemetry-arm-protocol-v1","status":"frozen-before-run","label":label,"iterations":iterations,"warmup":warmup,"global_policy":policy,"payload_seeds":payload_seeds,"channel_seeds":channel_seeds,"placement":{"gpu0":"four-cell home + two local NRx + Qwen","gpu1":"four-cell home + two local NRx + Qwen"},"shared_resources":"one serializable BurstGPT request queue; recovery, NRx endpoint/IPC, Qwen execution and physical credits remain per-home","transaction_order":"observe NRx -> certificate order -> bounded global prepare/hold -> local atomic replan+AI lease -> bounded global commit -> physical AI -> bounded global complete; broker fail-stop disables global AI clients without retry; local RAN certificates continue","fault_injection":{"home":0,"operation":operation,"operation_number":number,"point":"process calls os._exit(86) immediately after the configured state transition, before reply; attempted-RPC wall time includes the detecting call and excludes post-quarantine no-ops","expected_exit_code":86,"broker_rpc_timeout_ms":5},"cuda_module_loading":"LAZY","period_ms":180,"deadline_ms":155,"nrx_bound_ms":45,"conv_bound_ms":conv_bound_ms,"guard_ms":2,"global_broker_rpc_timeout_ms":5,"global_broker_transaction_budget_ms":15,"admission_ai_guard_ms":17,"fault_every":5,"rpc_telemetry_required":True,"source_sha256":{key:hashlib.sha256(path.read_bytes()).hexdigest() for key,path in zip(keys,paths)}}
...
for home in 0 1; do
    ...
    shifter_home_aerial "$home" env CUDA_MPS_CLIENT_PRIORITY=0 \
        python3 /softwall/global_trace_service_bound_fault_controller.py \
...
gates={
 ...
 'rpc_wall_bound':bool(rpc_records) and rpc_control['wall_timeout_exceeded']==0 and rpc_max<=p['global_broker_rpc_timeout_ms'],
}
value={'schema':'softwall-service-bound-fault-telemetry-arm-result-v1','protocol':str(protocol),'crash_operation':operation,'broker_exit_status':broker_status,'fault_detected_ns':fault_detected_ns,'target_request_id':target_request_id,'operational_global_queue_faults':operational_faults,'radio_records_after_fault_detection':radio_after,'homes':homes,'control_evidence':rpc_control,'gates':gates,'all_pass':all(gates.values()),'artifact_sha256':{str(path):hashlib.sha256(path.read_bytes()).hexdigest() for path in artifacts},'scope':f'wall-clock attempted-RPC telemetry for the 5 ms per-RPC and 15 ms fully admission-budgeted broker fail-stop after applying home0 {operation}30; all global AI clients fail closed while local RAN certificates continue'}
tmp=out.with_suffix('.tmp');tmp.write_text(json.dumps(value,indent=2));tmp.replace(out);print(json.dumps(gates,indent=2))
if not value['all_pass']:raise SystemExit('service-bound fault-telemetry arm failed')
PY
cleanup
trap - EXIT INT TERM
echo "service-bound fault-telemetry arm complete: $result"
