...
requal="$raw/confirm54_requalification_job${SLURM_JOB_ID}.json"
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    ...
    --seed 20321954 --snr-db -8.5 --channel-seed-base 20359900
python3 - "$requal" <<'PY'
import json, sys
x = json.load(open(sys.argv[1]))
if x['iterations'] != 200 or x['deadline_misses']:
    raise SystemExit('confirm54 operational requalification failed')
print('confirm54 operational requalification passed', flush=True)
PY

sleep 20
SOFTWALL_GPU_LOCK_HELD=1 ENDPOINT_CAP=80 BACKGROUND_KIND=nrx \
BACKGROUND_CAP=20 BACKGROUND_REPEATS=1 AI_BUDGET_MS=40 \
AI_GUARD_MS=2 AI_RPC_TIMEOUT_MS=35 PERIOD_MS=45 DEADLINE_MS=80 \
ENDPOINT_TIMEOUT_MS=30 NRX_BOUND_MS=50 CONV_BOUND_MS=25 \
COMMIT_GUARD_MS=2 RECOVERY_GAP_AI=1 FALLBACK_TIMING=early_if_clear \
WARMUP=20 SEED=20321053 SNR_DB=-8.5 CHANNEL_SEED_BASE=20353000 \
    bash "$SOFTWALL_SCRIPTS/run_same_request_transaction.sh" \
    "confirm54_retimed_job${SLURM_JOB_ID}" 1000

cleanup
trap - EXIT INT TERM
echo 'confirm54 retimed recovery canary completed'
