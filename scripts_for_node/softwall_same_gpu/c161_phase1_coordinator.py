#!/usr/bin/env python3
"""C161 phase-1 P180 physical fault coordinator.

The first physical closure covers a no-fault control, correlated suppression
of every observed NeuralRx success, and a forced latest-start non-launch.
"""
...
from c161_phase1_faults import arm_for_index, effective_successes, parse_arm_order
...
def main() -> None:
    ...
    parser.add_argument(
        "--arm-order",
        default="no_fault,correlated_all_fail,latest_start_nonlaunch",
    )
    parser.add_argument("--arm-length", type=int, default=30)
    parser.add_argument("--fault-hold-ms", type=float, default=10.0)
    args = parser.parse_args()
    if (args.iterations <= 0 or args.period_ms <= 0 or args.arm_length <= 0
            or args.fault_hold_ms <= 0):
        parser.error("iterations and period must be positive")
    try:
        arms = parse_arm_order(args.arm_order, args.iterations, args.arm_length)
    ...
    recoveries = []
    result = {
        "schema": "softwall-c161-phase1-coordinator-v1",
        "host": platform.node(),
    ...
    try:
        ...
        for index in range(args.iterations):
            sequence = index + 1
            arm = arm_for_index(index, arms, args.arm_length)
            context_length = CONTEXT_LENGTHS[index % len(CONTEXT_LENGTHS)]
            ...
            effective_timely_successes = effective_successes(
                arm, actual_timely_successes
            )
            (
                plan, revalidation_started_ns, revalidation_completed_ns,
                revalidation_attempts,
            ) = bounded_revalidate(
                lambda launch_now_ns: build_actual_outcome_batch_plan(
            ...
            dispatch = {
                "schema": "softwall-c159-q2-dispatch-v1",
                "sequence": sequence,
            ...
            if plan["ai_expected"]:
                ...
                if arm == "latest_start_nonlaunch":
                    hold_started_ns = time.perf_counter_ns()
                    time.sleep(args.fault_hold_ms / 1000.0)
                    injected_hold_ms = (
                        time.perf_counter_ns() - hold_started_ns
                    ) / 1e6
            ...
            rounds.append({
                ...
                "injected_host_hold_ms": injected_hold_ms,
                "success_keys": [list(key) for key in plan["success_keys"]],
