...
# V15 arm: fault prepare/commit/complete with only commit on the RAN critical path.
...
label=${SOFTWALL_SHARD_LABEL:-v15_single_token_control_fault_job${SLURM_JOB_ID}}
...
value={'schema':'softwall-v15-single-token-control-fault-arm-result-v1','protocol':str(protocol),'crash_operation':operation,'broker_exit_status':broker_status,'fault_detected_ns':fault_detected_ns,'target_request_id':target_request_id,'operational_global_queue_faults':operational_faults,'radio_records_after_fault_detection':radio_after,'homes':homes,'control_evidence':rpc_control,'gates':gates,'all_pass':all(gates.values()),'artifact_sha256':{str(path):hashlib.sha256(path.read_bytes()).hexdigest() for path in artifacts},'scope':f'V15 post-apply home0 {operation}30 fail-stop with asynchronous prepare/complete and one 7 ms launch-commit admission charge; local RAN certificates continue while ambiguous global tokens remain quarantined'}
tmp=out.with_suffix('.tmp');tmp.write_text(json.dumps(value,indent=2));tmp.replace(out);print(json.dumps(gates,indent=2))
if not value['all_pass']:raise SystemExit('v15 single-token control fault arm failed')
PY
cleanup
trap - EXIT INT TERM
echo "v15 single-token control fault arm complete: $result"
