...
# V11 physical mechanism arm: two four-cell homes consume only context-128
# requests under the fully control-budgeted certificate executor.
...
label=${SOFTWALL_SHARD_LABEL:-v11_ai40_candidate_job${SLURM_JOB_ID}}
...
keys=["/softwall/global_trace_fully_budgeted_fault_contained_controller.py","/softwall/global_trace_lease_broker.py","/softwall/same_request_ipc_worker.py","/softwall/trace_qwen_worker.py","/softwall_task1/isca_v2/dart_runtime.py","/softwall_task1/isca_v2/cuda_ipc_channel.py"]
paths=[Path(value) for value in sys.argv[11:]]
value={"schema":"softwall-v11-ai40-candidate-arm-protocol-v1","status":"frozen-before-run","label":label,"iterations":iterations,"warmup":warmup,"global_policy":policy,"payload_seeds":payload_seeds,"channel_seeds":channel_seeds,"trace":"/softwall_data/softwall_context128_mechanism_trace_v1.json","trace_sha256":"9c3ebdb3d7db3fec05048653529837c083115b0ec554280a79fea2dfbe565bf2","placement":{"gpu0":"four-cell home + two local NRx + Qwen","gpu1":"four-cell home + two local NRx + Qwen"},"shared_resources":"one serializable synthetic context128 request queue; recovery, NRx endpoint/IPC, Qwen execution and physical credits remain per-home","transaction_order":"observe NRx -> merge all ready recoveries in live-certificate order -> global hold -> local atomic recovery replan+AI lease -> global commit -> physical completion -> local retire+global complete","cuda_module_loading":"LAZY","period_ms":180,"deadline_ms":155,"nrx_bound_ms":45,"conv_bound_ms":conv_bound_ms,"guard_ms":2,"global_broker_rpc_timeout_ms":5,"global_broker_transaction_budget_ms":15,"admission_ai_guard_ms":17,"target_ai_bound_ms":40,"endpoint_ring_depth":1,"rejected_recovery_before_exchange":False,"fault_every":5,"source_sha256":{key:hashlib.sha256(path.read_bytes()).hexdigest() for key,path in zip(keys,paths)}}
...
for home in 0 1; do
    ...
    shifter_home_aerial "$home" env CUDA_MPS_CLIENT_PRIORITY=0 \
        python3 /softwall/global_trace_fully_budgeted_fault_contained_controller.py \
