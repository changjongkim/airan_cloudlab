...
label="c165_conventional_stage_profile_seed20359300_job${SLURM_JOB_ID}"
...
shifter_p2p python3 /softwall/c163_raw_p2p_controller.py \
    ...
    --output "$controller_output" --warmup 100 --iterations 300 \
    --deadline-ms 4.5 --seed 20359300 --cpu-affinity-index 0 \
    --profile-conventional >"$controller_log" 2>&1 &
controller_pid=$!

shifter_p2p python3 /softwall/c163_raw_p2p_nrx_worker.py \
    --tag "$tag" --ipc-dir "$state" \
    --engine /softwall_runtime/engines/neural_rx_fp16_full.trt \
    --output "$worker_output" --seed 20359300 \
    --source-device 0 --destination-device 1 \
    --same-stream --busy-poll --cpu-affinity-index -1 \
    --input-mode persistent_ordered >"$worker_log" 2>&1 &
worker_pid=$!
...
echo "C165 conventional stage diagnostic complete: $controller_output"
