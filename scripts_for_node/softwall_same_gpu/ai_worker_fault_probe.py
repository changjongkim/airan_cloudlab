#!/usr/bin/env python3
"""Cooperative or continuous CUDA work-unit worker for SoftWall experiments."""
...
def report(args: argparse.Namespace, records: list[dict], wall_s: float) -> dict:
    ...
    return {
        ...
        "response_delay_unit": args.response_delay_unit,
    ...
    try:
        ...
        with connection, connection.makefile("rwb", buffering=0) as channel:
            while True:
                ...
                if args.response_delay_ms and (
                    args.response_delay_unit == 0
                    or len(durations) == args.response_delay_unit
                ):
...
def main() -> None:
    ...
    parser.add_argument("--response-delay-unit", type=int, default=0)
    ...
    if args.response_delay_unit < 0:
        parser.error("response delay unit must be non-negative")
