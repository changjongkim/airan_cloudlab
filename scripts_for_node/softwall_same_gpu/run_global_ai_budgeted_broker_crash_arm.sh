...
# post-commit broker process crash and fail-closed RAN containment.
...
label=${SOFTWALL_SHARD_LABEL:-global_ai_budgeted_broker_crash_job${SLURM_JOB_ID}}
...
python3 - "$protocol" "$iterations" "$warmup" "$label" "$global_policy" "$conv_bound_ms" \
    "$SOFTWALL_SCRIPTS/global_trace_budgeted_fault_contained_controller.py" \
    "$SOFTWALL_SCRIPTS/deadline_fault_contained_global_trace_client.py" \
    "$SOFTWALL_SCRIPTS/low_perturbation_crash_global_trace_lease_broker.py" \
...
keys=["/softwall/global_trace_budgeted_fault_contained_controller.py","/softwall/deadline_fault_contained_global_trace_client.py","/softwall/low_perturbation_crash_global_trace_lease_broker.py","/softwall/same_request_ipc_worker.py","/softwall/trace_qwen_worker.py","/softwall_task1/isca_v2/dart_runtime.py","/softwall_task1/isca_v2/cuda_ipc_channel.py"]
paths=[Path(value) for value in sys.argv[7:]]
value={"schema":"softwall-global-ai-budgeted-broker-crash-arm-protocol-v1","status":"frozen-before-run","label":label,"iterations":iterations,"warmup":warmup,"global_policy":policy,"placement":{"gpu0":"four-cell home + two local NRx + Qwen","gpu1":"four-cell home + two local NRx + Qwen"},"shared_resources":"one serializable BurstGPT request queue; recovery, NRx endpoint/IPC, Qwen execution and physical credits remain per-home","transaction_order":"observe NRx -> certificate order -> global hold -> local atomic replan+AI lease -> global commit; broker fail-stop disables global AI clients without retry; local RAN certificates continue","fault_injection":{"home":0,"commit_number":30,"point":"process calls os._exit(86) immediately after broker applies commit, before reply; no synchronous fault-path logging","expected_exit_code":86,"broker_rpc_timeout_ms":5},"cuda_module_loading":"LAZY","period_ms":180,"deadline_ms":155,"nrx_bound_ms":45,"conv_bound_ms":conv_bound_ms,"guard_ms":2,"global_broker_rpc_timeout_ms":5,"global_broker_transaction_budget_ms":10,"admission_ai_guard_ms":12,"fault_every":5,"source_sha256":{key:hashlib.sha256(path.read_bytes()).hexdigest() for key,path in zip(keys,paths)}}
...
shifter_home_aerial 0 python3 /softwall/low_perturbation_crash_global_trace_lease_broker.py \
    ...
    --participants 2 --policy "$global_policy" --crash-after-commit-home 0 \
    --crash-after-commit-number 30 --output "$broker_output" &
broker_pid=$!
...
for home in 0 1; do
    ...
    shifter_home_aerial "$home" env CUDA_MPS_CLIENT_PRIORITY=0 \
        python3 /softwall/global_trace_budgeted_fault_contained_controller.py \
...
python3 - "$protocol" "$raw" "$label" "$warmup" "$broker_status" "$result" <<'PY'
import hashlib,json,sys
from collections import Counter
from pathlib import Path
protocol,raw,label,warmup,broker_status,out=Path(sys.argv[1]),Path(sys.argv[2]),sys.argv[3],int(sys.argv[4]),int(sys.argv[5]),Path(sys.argv[6])
homes=[];artifacts=[protocol];request_ids=[];controllers=[]
for home in (0,1):
...
commit_faults=[row for row in operational_faults[0] if row['operation']=='commit']
orphan_id=commit_faults[0]['request_id'] if len(commit_faults)==1 else None
...
gates={
 ...
 'control_budget_accounted':all(c['global_broker_transaction_budget_ms']==10 and c['admission_ai_guard_ms']==12 for c in controllers),
 'expected_broker_exit':broker_status==86,
 'home0_commit_ambiguity':len(commit_faults)==1 and orphan_id is not None,
 ...
 'ambiguous_request_not_executed':orphan_id not in request_ids,
...
value={'schema':'softwall-global-ai-budgeted-broker-crash-arm-result-v1','protocol':str(protocol),'broker_exit_status':broker_status,'fault_detected_ns':fault_detected_ns,'ambiguous_request_id':orphan_id,'operational_global_queue_faults':operational_faults,'radio_records_after_fault_detection':radio_after,'homes':homes,'gates':gates,'all_pass':all(gates.values()),'artifact_sha256':{str(path):hashlib.sha256(path.read_bytes()).hexdigest() for path in artifacts},'scope':'5 ms per-RPC and 10 ms admission-budgeted broker fail-stop after applying home0 commit30; no synchronous fault-path logging; all global AI clients fail closed while local RAN certificates continue'}
tmp=out.with_suffix('.tmp');tmp.write_text(json.dumps(value,indent=2));tmp.replace(out);print(json.dumps(gates,indent=2))
if not value['all_pass']:raise SystemExit('low-perturbation broker crash-containment arm failed')
PY
cleanup
trap - EXIT INT TERM
echo "global AI admission-budgeted broker crash arm complete: $result"
