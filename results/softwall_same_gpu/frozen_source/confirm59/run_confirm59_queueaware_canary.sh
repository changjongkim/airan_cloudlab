...
# Confirm59: request-owned PHY / queue-aware cross-endpoint clean canary.
...
require_allocation
payload_seed=20560001
channel_seed_base=20570000
iterations=100
prefix="confirm59_queueaware_job${SLURM_JOB_ID}"
...
if x['iterations'] != 200 or x['deadline_misses']:
    raise SystemExit('confirm59 operational requalification failed')
print('confirm59 operational requalification passed', flush=True)
PY
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/two_endpoint_queueaware_controller.py \
    ...
    --gate-mode always --gate-threshold 2.027073621749878 \
    --alternate-admission-order --clean-channel
...
cleanup
trap - EXIT INT TERM
echo 'confirm59 queue-aware cross-route canary completed'
