...
# Confirm67: exploratory Nsight capture inside one NeuralRx client and AI client.
...
require_allocation
mode=async_one
payload_seed=20630003
channel_seed_base=20631000
iterations=${SOFTWALL_GATE_ITERATIONS:-100}
prefix=${SOFTWALL_GATE_PREFIX:-confirm67_clients_job${SLURM_JOB_ID}}
...
worker1_pid=""
background_pid=""
tag="w${mode:0:1}_${SLURM_JOB_ID}_${BASHPID}"
...
if x['iterations'] != 200 or x['deadline_misses']:
    raise SystemExit('confirm65 operational requalification failed')
print('confirm65 operational requalification passed', flush=True)
PY
...
for cell in 0 1; do
    ...
    if [[ "$cell" == 0 ]]; then
        shifter_gpu env CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=40 CUDA_MPS_CLIENT_PRIORITY=1 TMPDIR="$state/tmp" \
            nsys profile --trace=cuda,nvtx --sample=none --cpuctxsw=none \
            --force-overwrite=true --output="$raw/${prefix}_worker0_profile" \
...
shifter_gpu env CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=20 CUDA_MPS_CLIENT_PRIORITY=1 TMPDIR="$state/tmp" \
    nsys profile --trace=cuda,nvtx --sample=none --cpuctxsw=none \
    --force-overwrite=true --output="$raw/${prefix}_background_profile" \
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/two_endpoint_async_ai_controller.py \
    ...
    --early-mandatory on --alternate-admission-order --ai-during-nrx "$mode"
...
cleanup
trap - EXIT INT TERM
echo "confirm65 mode=$mode completed"
