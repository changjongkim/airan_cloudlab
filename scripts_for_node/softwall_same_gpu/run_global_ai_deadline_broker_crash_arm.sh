...
label=${SOFTWALL_SHARD_LABEL:-global_ai_deadline_broker_crash_job${SLURM_JOB_ID}}
...
python3 - "$protocol" "$iterations" "$warmup" "$label" "$global_policy" "$conv_bound_ms" \
    "$SOFTWALL_SCRIPTS/global_trace_deadline_fault_contained_controller.py" \
...
keys=["/softwall/global_trace_deadline_fault_contained_controller.py","/softwall/deadline_fault_contained_global_trace_client.py","/softwall/low_perturbation_crash_global_trace_lease_broker.py","/softwall/same_request_ipc_worker.py","/softwall/trace_qwen_worker.py","/softwall_task1/isca_v2/dart_runtime.py","/softwall_task1/isca_v2/cuda_ipc_channel.py"]
paths=[Path(value) for value in sys.argv[7:]]
value={"schema":"softwall-global-ai-deadline-broker-crash-arm-protocol-v1","status":"frozen-before-run","label":label,"iterations":iterations,"warmup":warmup,"global_policy":policy,"placement":{"gpu0":"four-cell home + two local NRx + Qwen","gpu1":"four-cell home + two local NRx + Qwen"},"shared_resources":"one serializable BurstGPT request queue; recovery, NRx endpoint/IPC, Qwen execution and physical credits remain per-home","transaction_order":"observe NRx -> certificate order -> global hold -> local atomic replan+AI lease -> global commit; broker fail-stop disables global AI clients without retry; local RAN certificates continue","fault_injection":{"home":0,"commit_number":30,"point":"process calls os._exit(86) immediately after broker applies commit, before reply; no synchronous fault-path logging","expected_exit_code":86,"broker_rpc_timeout_ms":5},"cuda_module_loading":"LAZY","period_ms":180,"deadline_ms":155,"nrx_bound_ms":45,"conv_bound_ms":conv_bound_ms,"guard_ms":2,"global_broker_rpc_timeout_ms":5,"fault_every":5,"source_sha256":{key:hashlib.sha256(path.read_bytes()).hexdigest() for key,path in zip(keys,paths)}}
...
for home in 0 1; do
    ...
    shifter_home_aerial "$home" env CUDA_MPS_CLIENT_PRIORITY=0 \
        python3 /softwall/global_trace_deadline_fault_contained_controller.py \
...
cleanup
trap - EXIT INT TERM
echo "global AI deadline-bounded broker crash arm complete: $result"
