...
# Two disjoint four-cell RAN homes with synchronized releases on GPU0/GPU1.
...
label=${SOFTWALL_SHARD_LABEL:-sharded_two_home_job${SLURM_JOB_ID}}
...
python3 - "$protocol" "$iterations" "$warmup" "$label" \
...
keys=["/softwall/four_cell_trace_baseline_controller.py","/softwall/same_request_ipc_worker.py","/softwall/trace_qwen_worker.py","/softwall_task1/isca_v2/dart_runtime.py","/softwall_task1/isca_v2/cuda_ipc_channel.py"]
paths=[Path(value) for value in sys.argv[5:]]
value={"schema":"softwall-sharded-two-home-arm-protocol-v1","status":"frozen-before-run","label":label,"iterations":iterations,"warmup":warmup,"placement":{"gpu0":"four-cell home + two local NRx + Qwen","gpu1":"four-cell home + two local NRx + Qwen"},"shared_resources":"MPS control only; recovery, endpoint, IPC, Qwen and trace queues are disjoint by home","cuda_module_loading":"LAZY","period_ms":180,"deadline_ms":155,"nrx_bound_ms":45,"conv_bound_ms":12,"guard_ms":2,"fault_every":5,"source_sha256":{key:hashlib.sha256(path.read_bytes()).hexdigest() for key,path in zip(keys,paths)}}
...
cleanup() {
    ...
    rm -f "$SOFTWALL_ROOT/mps/$SLURM_JOB_ID/sh0_${BASHPID}.sock" "$SOFTWALL_ROOT/mps/$SLURM_JOB_ID/sh1_${BASHPID}.sock"
...
gates={'two_homes':len(homes)==2,'same_first_release':homes[0]['first_release_ns']==homes[1]['first_release_ns'],'eight_cells':sum(x['summary']['records'] for x in homes)==8*json.loads(protocol.read_text())['iterations'],'worker_count_match':all(w['count_match'] for h in homes for w in h['workers']),'system_safety':all(all(h['safety'].values()) for h in homes)}
value={'schema':'softwall-sharded-two-home-arm-result-v1','protocol':str(protocol),'homes':homes,'gates':gates,'all_pass':all(gates.values()),'artifact_sha256':{str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in artifacts},'scope':'disjoint-product two-home certificate gate; no shared endpoint or global AI lease'}
tmp=out.with_suffix('.tmp');tmp.write_text(json.dumps(value,indent=2));tmp.replace(out);print(json.dumps(gates,indent=2))
if not value['all_pass']:raise SystemExit('sharded two-home arm failed')
PY

cleanup
trap - EXIT INT TERM
echo "sharded two-home arm complete: $result"
