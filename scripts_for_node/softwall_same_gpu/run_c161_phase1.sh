...
# C161 phase-1 P180 physical fault semantics.
...
label=${SOFTWALL_C161_LABEL:-c161_phase1_job${SLURM_JOB_ID}}
campaign=${SOFTWALL_C161_CAMPAIGN:-development}
seed_base=${SOFTWALL_C161_SEED_BASE:-36000000}
excluded_nodes=${SOFTWALL_C161_EXCLUDED_NODES:-nid002288,nid002100,nid001109,nid001085,nid001064,nid001124,nid001177,nid001308,nid001204,nid001632}
iterations=${SOFTWALL_C161_ITERATIONS:-90}
arm_order=${SOFTWALL_C161_ARM_ORDER:-no_fault,correlated_all_fail,latest_start_nonlaunch}
arm_length=${SOFTWALL_C161_ARM_LENGTH:-30}
fault_hold_ms=${SOFTWALL_C161_FAULT_HOLD_MS:-10}
...
qwen_socket="$SOFTWALL_ROOT/mps/$SLURM_JOB_ID/c161p1.sock"
...
python3.11 "$SOFTWALL_SCRIPTS/build_c161_phase1_protocol.py" \
    ...
    --arm-order "$arm_order" --arm-length "$arm_length" \
    --fault-hold-ms "$fault_hold_ms" --period-ms "$period_ms" --warmup "$warmup" \
    --release-lead-ms "$release_lead_ms"

current_node=$(hostname)
case ",$excluded_nodes," in
    *",$current_node,"*)
        echo "C161 phase-1 node $current_node is in the frozen exclusion set" >&2
        exit 3
        ;;
esac
...
for _ in {1..3600}; do
    ...
    if ! kill -0 "$qwen_pid" 2>/dev/null; then
        wait "$qwen_pid" || true
        echo "C161 phase-1 Qwen worker exited before readiness" >&2
        exit 1
    fi
    sleep 0.05
done
[[ -S "$qwen_socket" ]] || { echo "C161 phase-1 Qwen readiness timeout" >&2; exit 1; }
...
shifter_all env CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=80 CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/c161_phase1_coordinator.py \
    --peer-spec "$peer_spec" --ipc-dir "$state" \
...
pids=""
[[ "$failure" -eq 0 ]] || {
    echo "C161 phase-1 process failure; artifacts preserved" >&2
    exit 1
}

python3.11 "$SOFTWALL_SCRIPTS/analyze_c161_phase1.py" \
...
cleanup
trap - EXIT INT TERM
echo "C161 phase-1 physical fault campaign complete: $result"
