...
label="c163_raw_p2p_v5_stage_profile_job${SLURM_JOB_ID}"
...
shifter_p2p python3 /softwall/c163_raw_p2p_controller.py \
    ...
    --deadline-ms 4.5 --seed 20358600 --cpu-affinity-index 0 \
    >"$controller_log" 2>&1 &
controller_pid=$!
...
shifter_p2p python3 /softwall/c163_raw_p2p_nrx_worker.py \
    --tag "$tag" --ipc-dir "$state" \
    --engine /softwall_runtime/engines/neural_rx_fp16_full.trt \
    --output "$worker_output" --seed 20358600 \
    --source-device 0 --destination-device 1 --same-stream --busy-poll \
    --profile-stages --cpu-affinity-index -1 >"$worker_log" 2>&1 &
worker_pid=$!
...
echo "C163 raw-P2P stage profile complete: $worker_output"
