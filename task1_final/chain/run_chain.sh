...
setup_topology_B() {
    # 3g + 3g + 1g on GPU 0
    log "MIG topology B: 3g + 3g + 1g on GPU 0"
    destroy_mig
    sudo nvidia-smi -i 0 -mig 1 > /dev/null 2>&1
    sleep 2
    sudo nvidia-smi mig -cgi 9,9,19 -C -i 0 > /tmp/mig_B.log 2>&1
    if [ $? -ne 0 ]; then
        alert "Topology B creation failed; see /tmp/mig_B.log"
        return 1
    fi
    # 3g partitions are for L1 and Qwen; 1g is for NRx
    MIGS_3g=($(nvidia-smi -L | awk '/MIG 3g/{print $6}' | tr -d ')'))
    MIG_L1=${MIGS_3g[0]}
    MIG_AI=${MIGS_3g[1]}
    MIG_NRX=$(nvidia-smi -L | awk '/MIG 1g/{print $6}' | tr -d ')')
    log "MIG_L1(3g)=$MIG_L1  MIG_AI(3g)=$MIG_AI  MIG_NRX(1g)=$MIG_NRX"
