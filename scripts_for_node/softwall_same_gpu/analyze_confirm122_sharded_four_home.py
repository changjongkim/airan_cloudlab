#!/usr/bin/env python3
"""Audit two prospectively frozen four-home/twelve-cell SoftWall arms."""
...
PROTOCOL = RES / "confirm122_sharded_four_home_protocol.json"
...
def main() -> None:
    ...
    arm_source_map = {
        "/softwall/sharded_trace_baseline_controller.py":
            "scripts_for_node/softwall_same_gpu/sharded_trace_baseline_controller.py",
    ...
    for spec in protocol["arms"]:
        ...
        arm_gates = {
            ...
            "twelve_cell_record_count": sum(home["records"] for home in homes)
                == 12 * protocol["iterations"],
            "all_safety": all(
        ...
        arms.append({
            ...
            "home_release_delta_ns": (
                max(home["first_release_ns"] for home in homes)
                - min(home["first_release_ns"] for home in homes)
            ),
    ...
    seed_tuples = {
        (
            arm["payload_seed_base"], arm["channel_seed_base"],
        )
    ...
    output = {
        "schema": "softwall-confirm122-sharded-four-home-v1",
        ...
        "scope": (
            "four disjoint three-cell recovery homes on four physical A100 GPUs, "
    ...
    out = RES / "confirm122_sharded_four_home_result.json"
    temporary = out.with_suffix(".tmp")
    temporary.write_text(json.dumps(output, indent=2), encoding="utf-8")
    temporary.replace(out)
    print(json.dumps(gates, indent=2))
    if not output["all_pass"]:
        raise SystemExit("Confirm122 sharded four-home gate failed")
