#!/usr/bin/env bash
# Part 7 statistical only: 10 trials at N=5, 6, 7 with MPS on (breakdown zone)
set -uo pipefail
GPU=${GPU:-0}
CELLS=${CELLS:-20}
...
OUT=/mydata/results/$DATE_DIR/chain18_p7
...
start_qwen_cross() {
  ...
  docker run -d --rm --init --user 0:0 --gpus "device=$u" \
    ...
    --entrypoint bash -v "$HF_CACHE:/hf" -e HF_HOME=/hf \
...
log "=== Part 7 statistical (10 trials at N=5,6,7 MPS on) ==="
setup_A
for N in 5 6 7; do
  mps_start "$UUID_L1"
  ...
  for t in $(seq 1 $N_TRIALS); do
    ...
    start_nrx_multi "$UUID_L1" "${LABEL}_same_t${t}" $N
    sleep 15
...
mig_off
log "=== Part 7 stat DONE ==="
touch /users/sgkim/CHAIN18_PART7STAT_DONE
