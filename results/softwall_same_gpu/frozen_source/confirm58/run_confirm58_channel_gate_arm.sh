...
# Confirm58: one pinned two-cell online channel-gate baseline arm.
...
require_allocation
pair=${1:?pair must be 1 or 2}
mode=${2:?mode must be always or threshold}
case "$pair:$mode" in
    1:always|1:threshold|2:always|2:threshold) ;;
    *) echo "invalid pair/mode: $pair:$mode" >&2; exit 2 ;;
esac
if [[ "$pair" == 1 ]]; then
    payload_seed=20500001
    channel_seed_base=20510000
else
    payload_seed=20500002
    channel_seed_base=20520000
fi
iterations=${SOFTWALL_GATE_ITERATIONS:-1000}
prefix=${SOFTWALL_GATE_PREFIX:-confirm58_pair${pair}_${mode}_job${SLURM_JOB_ID}}
...
worker1_pid=""
background_pid=""
tag="g${pair}${mode:0:1}_${SLURM_JOB_ID}_${BASHPID}"
...
if x['iterations'] != 200 or x['deadline_misses']:
    raise SystemExit('confirm58 operational requalification failed')
print('confirm58 operational requalification passed', flush=True)
PY
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/two_endpoint_channel_gate_controller.py \
    ...
    --gate-mode "$mode" --gate-threshold 2.027073621749878
...
cleanup
trap - EXIT INT TERM
echo "confirm58 pair=$pair mode=$mode completed"
