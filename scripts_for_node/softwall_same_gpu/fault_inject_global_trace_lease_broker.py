#!/usr/bin/env python3
"""Serializable global AI-request leases for sharded SoftWall homes."""
...
class BrokerServer(socketserver.ThreadingUnixStreamServer):
    ...
    def __init__(self, path: str, state: GlobalTraceLeaseState,
                 drop_commit_response_home: int | None = None,
                 drop_commit_response_number: int | None = None):
        self.state = state
        self.stop_requested = False
        self.drop_commit_response_home = drop_commit_response_home
        self.drop_commit_response_number = drop_commit_response_number
        self.commit_counts = {}
        self.faults = []
        self.fault_lock = threading.Lock()
        super().__init__(path, BrokerHandler)

    def should_drop_commit_response(self, request: dict, result: dict) -> bool:
        """Apply a commit once, then lose exactly the configured reply.
        ...
        with self.fault_lock:
            ...
            if (home != self.drop_commit_response_home
                    or count != self.drop_commit_response_number):
                return False
            self.faults.append({
                "fault": "drop_commit_response_after_apply",
                "home": home,
                "commit_number": count,
                "token": request["token"],
        ...
        while True:
            ...
            try:
                ...
                if op == "init":
                    ...
                    if self.server.should_drop_commit_response(request, result):
                        return
                elif op == "abort":
...
def main() -> None:
    ...
    parser.add_argument("--drop-commit-response-home", type=int)
    parser.add_argument("--drop-commit-response-number", type=int)
    ...
    if ((args.drop_commit_response_home is None)
            != (args.drop_commit_response_number is None)):
        parser.error("both commit-response fault arguments are required")
    if args.drop_commit_response_number is not None and args.drop_commit_response_number <= 0:
        parser.error("commit-response fault number must be positive")
    if (args.drop_commit_response_home is not None
            and not 0 <= args.drop_commit_response_home < args.participants):
        parser.error("commit-response fault home is outside participant range")
    server = BrokerServer(
        str(socket_path), state,
        args.drop_commit_response_home, args.drop_commit_response_number,
    )
    try:
        ...
        output["fault_configuration"] = {
            "drop_commit_response_home": args.drop_commit_response_home,
            "drop_commit_response_number": args.drop_commit_response_number,
        }
