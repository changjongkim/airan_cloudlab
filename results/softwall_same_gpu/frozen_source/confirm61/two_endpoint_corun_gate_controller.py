#!/usr/bin/env python3
"""Two-cell gate with optional early mandatory/Nrx host co-run probe.
...
pinned and AI runs after radio. The early arm starts conventional processing
for a skipped cell while the other cell's NRx is outstanding. Host overlap is
observable; kernel overlap and WCET remain separate questions.
"""
...
def main() -> None:
    ...
    parser.add_argument("--gate-mode", choices=("always", "threshold"), required=True)
    ...
    try:
        ...
        for index in range(args.iterations):
            ...
            for cell, tx in enumerate(transactions):
                ...
                row["commit_return_ns"] = commit_return_ns
            for row in radio:
                if row["gate_skipped"] and args.early_mandatory == "on":
