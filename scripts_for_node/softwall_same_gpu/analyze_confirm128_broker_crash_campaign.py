#!/usr/bin/env python3
"""Aggregate prospective C128 broker fail-stop containment arms."""
...
def main() -> None:
    ...
    for path in args.arms:
        ...
        totals["radio_records_after_crash"] += sum(arm["radio_records_after_crash"])
        arms.append({
            "path": str(path),
    ...
    gates = {
        ...
        "both_homes_continue_radio": all(
            arm["gates"]["both_homes_radio_after_crash"] for arm in arms
        ),
    ...
    value = {
        "schema": "softwall-confirm128-broker-crash-campaign-v1",
        ...
        "claim": (
            "After a broker fail-stop immediately following one applied commit, "
    ...
    if not value["all_pass"]:
        raise SystemExit("C128 campaign gate failed")
