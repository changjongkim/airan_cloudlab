...
# Confirm103: controlled one-versus-two NeuralRx count probe.
...
prefix=${SOFTWALL_GATE_PREFIX:-confirm103_nrx_count${SOFTWALL_MAX_NRX_ADMISSIONS:?}_seed${channel_seed_base}_job${SLURM_JOB_ID}}
...
shifter_qwen env CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=20 CUDA_MPS_CLIENT_PRIORITY=1 \
    python3 /softwall/qwen_worker.py --mode rpc --phase prefill \
    --model Qwen/Qwen2.5-1.5B --context-length 16 --batch-size 1 \
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/four_cell_nrx_count_probe_controller.py \
    ...
    --nrx-bound-ms "${SOFTWALL_NRX_BOUND_MS:?}" --conv-bound-ms "${SOFTWALL_CONV_BOUND_MS:?}" --commit-guard-ms 2 \
    --endpoint-timeout-ms 100 --ai-budget-ms 50 --ai-guard-ms 2 \
    --ai-rpc-timeout-ms 45 --seed "$payload_seed" --snr-db -8.5 \
    --channel-seed-base "$channel_seed_base" \
    --gate-mode always --gate-threshold 1.9490545988082886 \
    --max-nrx-admissions "$SOFTWALL_MAX_NRX_ADMISSIONS" \
    --gc-mode "$SOFTWALL_GC_MODE" \
    --early-mandatory on --alternate-admission-order --ai-during-nrx async_one \
    --ai-aware-recovery joint --ai-deadline-ms 153 \
...
cleanup
trap - EXIT INT TERM
echo "confirm103 controlled NeuralRx-count Qwen run completed"
