...
# Confirm64: fresh-seed reverse-order unresolved AI replication arm.
...
payload_seed=20620002
channel_seed_base=20621000
iterations=${SOFTWALL_GATE_ITERATIONS:-200}
prefix=${SOFTWALL_GATE_PREFIX:-confirm64_${mode}_job${SLURM_JOB_ID}}
...
worker1_pid=""
background_pid=""
tag="v${mode:0:1}_${SLURM_JOB_ID}_${BASHPID}"
...
if x['iterations'] != 200 or x['deadline_misses']:
    raise SystemExit('confirm64 operational requalification failed')
print('confirm64 operational requalification passed', flush=True)
PY
...
cleanup
trap - EXIT INT TERM
echo "confirm64 mode=$mode completed"
