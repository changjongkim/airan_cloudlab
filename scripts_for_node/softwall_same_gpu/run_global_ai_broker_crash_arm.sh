...
label=${SOFTWALL_SHARD_LABEL:-global_ai_broker_crash_job${SLURM_JOB_ID}}
...
crash_marker="$raw/${label}_broker_crash_marker.json"
...
rm -f "$result" "$broker_socket" "$broker_output" "$crash_marker"
...
python3 - "$protocol" "$iterations" "$warmup" "$label" "$global_policy" "$conv_bound_ms" \
    ...
    "$SOFTWALL_SCRIPTS/crash_inject_global_trace_lease_broker.py" \
...
shifter_home_aerial 0 python3 /softwall/crash_inject_global_trace_lease_broker.py \
    ...
    --crash-after-commit-number 30 --crash-marker "$crash_marker" --output "$broker_output" &
broker_pid=$!
...
[[ -f "$crash_marker" ]] || { echo "broker crash marker missing" >&2; exit 1; }
[[ ! -f "$broker_output" ]] || { echo "broker unexpectedly reached graceful output" >&2; exit 1; }
pids=""

python3 - "$protocol" "$raw" "$label" "$warmup" "$crash_marker" "$broker_status" "$result" <<'PY'
import hashlib,json,sys
from collections import Counter
from pathlib import Path
protocol,raw,label,warmup,marker_path,broker_status,out=Path(sys.argv[1]),Path(sys.argv[2]),sys.argv[3],int(sys.argv[4]),Path(sys.argv[5]),int(sys.argv[6]),Path(sys.argv[7])
homes=[];artifacts=[protocol,marker_path];request_ids=[];controllers=[]
for home in (0,1):
...
marker=json.loads(marker_path.read_text());fault=marker['fault'];p=json.loads(protocol.read_text())
orphan_id=fault['request_id'];fault_ns=fault['injected_ns']
radio_after=[]
for c in controllers:
 radio_after.append(sum(row['commit_return_ns']>fault_ns for row in c['records']))
operational_faults=[]
for c in controllers:
...
gates={
 ...
 'expected_broker_exit':broker_status==86 and marker['exit_code']==86 and fault['home']==0 and fault['commit_number']==30,
 ...
 'both_homes_radio_after_crash':all(value>0 for value in radio_after),
...
value={'schema':'softwall-global-ai-broker-crash-arm-result-v1','protocol':str(protocol),'crash_marker':str(marker_path),'crash':marker,'broker_exit_status':broker_status,'operational_global_queue_faults':operational_faults,'radio_records_after_crash':radio_after,'homes':homes,'gates':gates,'all_pass':all(gates.values()),'artifact_sha256':{str(path):hashlib.sha256(path.read_bytes()).hexdigest() for path in artifacts},'scope':'broker process fail-stop after applying one commit; all global AI clients fail closed without retry while disjoint local RAN certificates continue'}
tmp=out.with_suffix('.tmp');tmp.write_text(json.dumps(value,indent=2));tmp.replace(out);print(json.dumps(gates,indent=2))
if not value['all_pass']:raise SystemExit('global AI broker crash-containment arm failed')
PY

cleanup
trap - EXIT INT TERM
echo "global AI broker crash-containment arm complete: $result"
