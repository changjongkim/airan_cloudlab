...
# V15 physical mechanism arm: prepare/complete are deferred off the RAN path;
# only the launch-authorizing commit carries a 7 ms admission charge.
...
label=${SOFTWALL_SHARD_LABEL:-v15_ai45_single_token_job${SLURM_JOB_ID}}
...
keys=["/softwall/global_trace_fully_budgeted_fault_contained_controller.py","/softwall/global_trace_single_token_pipelined_controller.py","/softwall/single_token_pipelined_global_trace_client.py","/softwall/pipelined_global_trace_client.py","/softwall/test_single_token_pipelined_global_trace_client.py","/softwall/global_trace_lease_broker.py","/softwall/same_request_ipc_worker.py","/softwall/trace_qwen_worker.py","/softwall_task1/isca_v2/dart_runtime.py","/softwall_task1/isca_v2/cuda_ipc_channel.py"]
paths=[Path(value) for value in sys.argv[11:]]
value={"schema":"softwall-v15-ai45-single-token-arm-protocol-v1","status":"frozen-before-run","label":label,"iterations":iterations,"warmup":warmup,"global_policy":policy,"payload_seeds":payload_seeds,"channel_seeds":channel_seeds,"trace":"/softwall_data/softwall_context64_mechanism_trace_v1.json","trace_sha256":"b14388c7356878458031d4e881a0510f42f1b4414493f876cd13d2b881979277","placement":{"gpu0":"four-cell home + two local NRx + Qwen","gpu1":"four-cell home + two local NRx + Qwen"},"shared_resources":"one serializable synthetic context64 request queue; recovery, NRx endpoint/IPC, Qwen execution and physical credits remain per-home","transaction_order":"asynchronous global prepare -> current-horizon revalidation -> local atomic recovery replan+AI lease -> synchronous global commit ACK -> physical AI -> local fence/retire -> asynchronous global complete","ownership_contract":"at-most-one-staged-or-offered-token-per-home","cuda_module_loading":"LAZY","period_ms":180,"deadline_ms":155,"nrx_bound_ms":45,"conv_bound_ms":conv_bound_ms,"guard_ms":2,"global_broker_rpc_timeout_ms":5,"global_broker_rpc_admission_bound_ms":7,"global_broker_transaction_budget_ms":7,"admission_ai_guard_ms":9,"target_ai_bound_ms":45,"effective_transaction_bound_ms":54,"endpoint_ring_depth":1,"rejected_recovery_before_exchange":False,"fault_every":5,"rpc_telemetry_required":True,"source_sha256":{key:hashlib.sha256(path.read_bytes()).hexdigest() for key,path in zip(keys,paths)}}
...
for home in 0 1; do
    ...
    shifter_home_aerial "$home" env CUDA_MPS_CLIENT_PRIORITY=0 \
        python3 /softwall/global_trace_single_token_pipelined_controller.py \
        ...
        --trace-sha256 b14388c7356878458031d4e881a0510f42f1b4414493f876cd13d2b881979277 \
        --system softwall --ai-bound-map 64:45 \
...
for c in controllers:
 ...
 for decision in c['recovery_decisions']:
  ...
  if branch and decision.get('ai_lease') and record is not None and record['bound_ms']==45.0:
...
rpc_max=max((row['elapsed_ms'] for row in rpc_records),default=None);commit_records=[row for row in rpc_records if row['operation']=='commit'];handoffs=[row for telemetry in rpc_by_home if telemetry is not None for row in telemetry.get('handoffs',[])]
rpc_control={'records':len(rpc_records),'by_operation':dict(Counter(row['operation'] for row in rpc_records)),'max_elapsed_ms':rpc_max,'faulted':sum(row['faulted'] for row in rpc_records),'commit_records':len(commit_records),'commit_max_elapsed_ms':max((row['elapsed_ms'] for row in commit_records),default=None),'commit_admission_bound_exceeded':sum(row['elapsed_ms']>p['global_broker_rpc_admission_bound_ms'] for row in commit_records),'handoffs':len(handoffs),'handoff_max_elapsed_ms':max((row['elapsed_ms'] for row in handoffs),default=None)}
gates={'two_homes':len(homes)==2,'same_first_release':homes[0]['first_release_ns']==homes[1]['first_release_ns'],'eight_cells':sum(x['summary']['records'] for x in homes)==8*p['iterations'],'worker_count_match':all(w['count_match'] for h in homes for w in h['workers']),'system_safety':all(all(h['safety'].values()) for h in homes),'broker_homes':summary['registered_homes']==[0,1] and summary['finalized_homes']==[0,1],'global_unique_requests':len(request_ids)==len(set(request_ids))==summary['timely_requests'],'broker_drained':summary['outstanding_tokens']==0 and summary['duplicate_commit_count']==0,'both_homes_ai':all(h['summary']['ai_timely_value_tokens']>0 for h in homes),'physical_ring_depth_conformance':max_admitted<=2*p['endpoint_ring_depth'],'candidate_branch_coverage':candidate_branches>0,'ai45_candidate_exchange_observed':candidate_exchanges>0,'candidate_horizon_margin_nonnegative':bool(candidate_margins) and min(candidate_margins)>=0,'control_budget_accounted':all(c['global_broker_transaction_budget_ms']==7 and c['admission_ai_guard_ms']==9 for c in controllers),'rpc_telemetry_present':all(value is not None and value.get('schema')=='softwall-pipelined-broker-rpc-telemetry-v1' for value in rpc_by_home),'only_commit_declared_critical':all(value.get('critical_path_operations')==['commit'] and set(value.get('deferred_operations',[]))=={'prepare','abort','complete'} for value in rpc_by_home),'single_unlaunched_token':all(value.get('ownership_contract')==p['ownership_contract'] and value.get('ownership_evidence',{}).get('maximum_unlaunched_tokens',2)<=1 and value.get('state',{}).get('ownership_invariant_holds') for value in rpc_by_home),'retained_token_branch_exercised':all(value.get('ownership_evidence',{}).get('suppressed_prepare_due_owned_token',0)>0 for value in rpc_by_home),'rpc_prepare_commit_complete_coverage':{'prepare','commit','complete'}<=set(row['operation'] for row in rpc_records),'commit_admission_wall_bound':bool(commit_records) and rpc_control['faulted']==0 and rpc_control['commit_admission_bound_exceeded']==0,'deferred_complete_not_mislabeled_ack':all(not row.get('global_complete_confirmed',True) for c in controllers for row in c['background_records'])}
value={'schema':'softwall-v15-ai45-single-token-arm-result-v1','protocol':str(protocol),'broker':str(broker_path),'broker_summary':summary,'homes':homes,'candidate_evidence':{'branches':candidate_branches,'exchanges':candidate_exchanges,'minimum_reserved_horizon_margin_ms':min(candidate_margins) if candidate_margins else None,'maximum_admitted_nrx_per_home_release':max_admitted},'control_evidence':rpc_control,'gates':gates,'all_pass':all(gates.values()),'artifact_sha256':{str(path):hashlib.sha256(path.read_bytes()).hexdigest() for path in artifacts},'scope':'synthetic context64-only mechanism qualification for exchange-only AI45 with one 7 ms launch-commit charge and 2 ms completion guard; prepare/complete execute on a separate control connection across two disjoint four-cell recovery homes'}
tmp=out.with_suffix('.tmp');tmp.write_text(json.dumps(value,indent=2));tmp.replace(out);print(json.dumps(gates,indent=2))
if not value['all_pass']:raise SystemExit('v15 AI45 single-token arm failed')
PY

cleanup
trap - EXIT INT TERM
echo "v15 AI45 single-token arm complete: $result"
