...
# Confirm63: one bounded AI unit before NRx observation diagnostic arm.
...
require_allocation
mode=${1:?mode must be off or one}
case "$mode" in off|one) ;; *) echo "invalid mode: $mode" >&2; exit 2 ;; esac
payload_seed=20610001
channel_seed_base=20611000
iterations=${SOFTWALL_GATE_ITERATIONS:-200}
prefix=${SOFTWALL_GATE_PREFIX:-confirm63_${mode}_job${SLURM_JOB_ID}}
...
worker1_pid=""
background_pid=""
tag="u${mode:0:1}_${SLURM_JOB_ID}_${BASHPID}"
...
if x['iterations'] != 200 or x['deadline_misses']:
    raise SystemExit('confirm63 operational requalification failed')
print('confirm63 operational requalification passed', flush=True)
PY
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/two_endpoint_unresolved_ai_controller.py \
...
cleanup
trap - EXIT INT TERM
echo "confirm63 mode=$mode completed"
