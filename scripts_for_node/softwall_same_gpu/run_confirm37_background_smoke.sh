...
BACKGROUND_CAP=20 BACKGROUND_REPEATS=1 AI_BUDGET_MS=6 AI_GUARD_MS=2 \
AI_RPC_TIMEOUT_MS=6 PERIOD_MS=90 DEADLINE_MS=80 \
ENDPOINT_TIMEOUT_MS=30 WARMUP=20 SEED=20321037 SNR_DB=-8.5 \
CHANNEL_SEED_BASE=20327000 \
    bash "$SOFTWALL_SCRIPTS/run_same_request_ipc.sh" \
    confirm37_external_background_smoke 500
...
prefix = 'confirm37_external_background_smoke_cap80_job' + job
...
report = {
    ...
    'campaign': 'confirm37_external_background_smoke',
    'job': job,
...
(result / 'confirm37_external_background_smoke.json').write_text(
    json.dumps(report, indent=2) + '\n'
)
(result / 'confirm37_external_background_smoke.md').write_text(
...
print((result / 'confirm37_external_background_smoke.md').read_text())
if not gates['all_pass']:
    raise SystemExit('confirm37 background smoke failed')
PY

cleanup
trap - EXIT INT TERM
echo 'confirm37 background smoke completed'
