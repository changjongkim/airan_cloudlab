...
label="c163_raw_p2p_v3_single_stream_job${SLURM_JOB_ID}"
...
shifter_p2p python3 /softwall/c163_raw_p2p_controller_v3.py \
    ...
    --deadline-ms 4.5 --seed 20358300 >"$controller_log" 2>&1 &
controller_pid=$!

shifter_p2p python3 /softwall/c163_raw_p2p_nrx_worker_v3.py \
    --tag "$tag" --ipc-dir "$state" \
    --engine /softwall_runtime/engines/neural_rx_fp16_full.trt \
    --output "$worker_output" --seed 20358300 \
    --source-device 0 --destination-device 1 >"$worker_log" 2>&1 &
worker_pid=$!
...
echo "C163 raw-P2P single-stream diagnostic complete: $controller_output"
