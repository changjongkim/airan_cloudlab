...
Two-endpoint same-GPU combined baseline: low-feature gate, queue routing, early mandatory.
...
def main() -> None:
    ...
    parser.add_argument("--gate-mode", choices=("low_threshold", "value_bin"), required=True)
    ...
    parser.add_argument("--gate-upper", type=float, required=True)
    ...
    if args.gate_upper <= args.gate_threshold:
        parser.error("gate upper edge must exceed lower edge")
    if min(
    ...
    try:
        ...
        for index in range(args.iterations):
            ...
            for cell in range(2):
                ...
                power = feature["channel_estimate_power"]
                gate_skipped = (
                    power < args.gate_threshold
                    or (args.gate_mode == "value_bin" and power >= args.gate_upper)
                )
