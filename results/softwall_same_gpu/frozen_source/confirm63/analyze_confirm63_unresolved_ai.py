...
"""Frozen audit of one bounded AI unit while NeuralRx remains unobserved."""
...
def main() -> None:
    ...
    protocol = read(base / "confirm63_unresolved_ai_protocol.json")
    gates: dict[str, bool] = {}
    ...
    arms = {}
    for mode in ("off", "one"):
        prefix = f"confirm63_{mode}_job{args.job}"
    ...
    report = {
        "schema": "softwall-confirm63-unresolved-ai-v1",
        "job": args.job,
        "gates": gates,
    ...
    output = base / f"confirm63_unresolved_ai_job{args.job}.json"
