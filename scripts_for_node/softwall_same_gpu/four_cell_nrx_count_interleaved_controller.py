#!/usr/bin/env python3
"""Within-process matched one-versus-two NeuralRx co-run probe.
...
def main() -> None:
    ...
    parser.add_argument("--max-nrx-admissions", type=int, choices=(1, 2), default=2)
    parser.add_argument("--nrx-count-pattern", choices=("1221",), required=True)
    ...
    try:
        ...
        for index in range(args.iterations):
            ...
            release_nrx_limit = int(args.nrx_count_pattern[index % len(args.nrx_count_pattern)])
            matched_pair_index = index // 2
            ...
            for cell, ctx in enumerate(contexts):
                if args.clean_channel:
                    ...
                    ctx["receiver"].apply_rayleigh_awgn(
                    args.snr_db,
                    args.channel_seed_base + matched_pair_index * args.cells + cell,
                        noise_reference="pre_fading",
                    )
            ...
            for cell in range(args.cells):
                ...
                radio.append({
                    ...
                    "channel_seed": args.channel_seed_base + matched_pair_index * args.cells + cell,
                    "cutoff_ns": cutoff_ns, "endpoint_id": None,
                    "admitted": False, "nrx_correct": None,
                    "policy_capped": False,
                    "nrx_admission_limit": release_nrx_limit,
            ...
            for cell in admission_order:
                ...
                if row["gate_skipped"]:
                    continue
                if admitted_count >= release_nrx_limit:
                    row["policy_capped"] = True
                    continue
    ...
    result = {
        "schema": "softwall-four-cell-gc-intervention-v1",
        ...
        "max_nrx_admissions": args.max_nrx_admissions,
        "nrx_count_pattern": args.nrx_count_pattern,
        ...
        "policy_caps": sum(row["policy_capped"] for row in records),
    ...
    print(
        f"[FOUR-CELL/GC-INTERVENTION] correct={result['correct_cells']}/{len(records)} "
