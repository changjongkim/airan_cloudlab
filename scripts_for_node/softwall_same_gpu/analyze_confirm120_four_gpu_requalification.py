...
PROTOCOL = RES / "confirm120_four_gpu_requalification_protocol.json"
...
def main() -> None:
    ...
    result = {
        "schema": "softwall-confirm120-four-gpu-requalification-v1",
    ...
    output = RES / "confirm120_four_gpu_requalification_job58815435.json"
    temporary = output.with_suffix(".tmp")
    temporary.write_text(json.dumps(result, indent=2))
    temporary.replace(output)
    print(json.dumps(gates, indent=2))
    if not result["all_pass"]:
        raise SystemExit("Confirm120 four-GPU requalification failed")
