...
# Confirm70: node-level GPU trace of early conventional versus other-cell NRx.
...
require_allocation
mode=on
payload_seed=20580001
channel_seed_base=20581000
iterations=${SOFTWALL_GATE_ITERATIONS:-40}
prefix=${SOFTWALL_GATE_PREFIX:-confirm70_nodes_job${SLURM_JOB_ID}}
...
worker1_pid=""
background_pid=""
tag="c${mode:0:1}_${SLURM_JOB_ID}_${BASHPID}"
...
if x['iterations'] != 200 or x['deadline_misses']:
    raise SystemExit('confirm61 operational requalification failed')
print('confirm61 operational requalification passed', flush=True)
PY
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 TMPDIR="$state/tmp" \
    nsys profile --trace=cuda,nvtx --cuda-graph-trace=node --sample=none --cpuctxsw=none \
    --force-overwrite=true --output="$raw/${prefix}_controller_profile" \
    python3 /softwall/two_endpoint_corun_gate_controller.py \
    ...
    --gate-mode threshold --gate-threshold 2.027073621749878 \
    --early-mandatory "$mode"
...
cleanup
trap - EXIT INT TERM
echo "confirm70 mode=$mode completed"
