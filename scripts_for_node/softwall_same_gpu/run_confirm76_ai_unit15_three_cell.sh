...
# Confirm76: qualify 15 ms host-budget AI units in three-cell joint admission.
...
require_allocation
payload_seed=${SOFTWALL_PAYLOAD_SEED:-20760001}
channel_seed_base=${SOFTWALL_CHANNEL_SEED_BASE:-20761000}
iterations=${SOFTWALL_GATE_ITERATIONS:-500}
failure_every=${SOFTWALL_CORRELATED_FAILURE_EVERY:-5}
prefix=${SOFTWALL_GATE_PREFIX:-confirm76_ai15_seed${channel_seed_base}_job${SLURM_JOB_ID}}
...
worker1_pid=""
background_pid=""
tag="a15_${SLURM_JOB_ID}_${BASHPID}"
...
if x['iterations'] != 200 or x['deadline_misses']:
    raise SystemExit('confirm76 operational requalification failed')
print('confirm76 operational requalification passed', flush=True)
PY
...
cleanup
trap - EXIT INT TERM
echo "confirm76 AI-15 three-cell run completed"
