#!/usr/bin/env python3
"""Persistent RAN-home owner for the C158 repeated actual-NRx pilot."""
...
def main() -> None:
    ...
    records = []
    result = {
        "schema": "softwall-actual-nrx-repeated-owner-v1",
        "host": platform.node(),
    ...
    try:
        ...
        atomic_json(args.ready_file, {
            "schema": "softwall-actual-nrx-repeated-owner-ready-v1",
            "key": key,
            "ready": True,
        })
        ...
        for index in range(args.iterations):
            ...
            outcome = {
                "schema": "softwall-actual-nrx-repeated-outcome-v1",
                "sequence": sequence,
                "key": key,
            ...
            # Synthetic channel generation and the validation-only local
            # conventional oracle are not part of the qualified radio path.
            # Keep preparation for the next request outside this request's
            # [release, expiry] interval so it cannot perturb the measured
            # NRx/Qwen/recovery transaction.
            if sequence < args.iterations:
                wait_until_ns(release_ns + round(args.expiry_ms * 1e6))
