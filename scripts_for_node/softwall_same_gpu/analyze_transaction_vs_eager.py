...
def main() -> None:
    ...
    requal = read(args.raw / f"confirm45_requalification_job{args.job}.json")
    gates: dict[str, bool] = {
        "requalification": (
    ...
    for round_spec in radio["rounds"]:
        ...
        eager_prefix = args.raw / (
            f"confirm45_r{round_id}_eager_job{args.job}_r1_eager_dual_job{args.job}"
        )
        external_prefix = args.raw / f"confirm45_r{round_id}_external_cap80_job{args.job}"
    ...
    report = {
        "schema": "softwall-confirm45-transaction-vs-eager-v1",
        "job": args.job,
    ...
    for row in rows:
        ...
        lines.append(
            ...
            f"{row['external_background_units']} | {row['external_recovery_gap_ai_units']} | {gain:+d} |"
        )
