...
# Confirm83: four-cell B_conv8 calendar qualification, only after Confirm82 screen.
...
prefix=${SOFTWALL_GATE_PREFIX:-confirm83_conv8_seed${channel_seed_base}_job${SLURM_JOB_ID}}
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/four_cell_atomic_lease_controller.py \
    ...
    --nrx-bound-ms 50 --conv-bound-ms 8 --commit-guard-ms 2 \
...
cleanup
trap - EXIT INT TERM
echo "confirm83 B_conv8 four-cell run completed"
