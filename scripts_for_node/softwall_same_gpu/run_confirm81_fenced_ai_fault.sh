...
# Confirm81: delayed AI RPC response after a GPU-synchronized joint marker.
...
require_allocation
payload_seed=${SOFTWALL_PAYLOAD_SEED:-20810051}
channel_seed_base=${SOFTWALL_CHANNEL_SEED_BASE:-20811000}
iterations=${SOFTWALL_GATE_ITERATIONS:-100}
failure_every=${SOFTWALL_CORRELATED_FAILURE_EVERY:-5}
prefix=${SOFTWALL_GATE_PREFIX:-confirm81_fenced_fault_seed${channel_seed_base}_job${SLURM_JOB_ID}}
...
worker1_pid=""
background_pid=""
tag="f4f81_${SLURM_JOB_ID}_${BASHPID}"
background_socket="$SOFTWALL_ROOT/mps/$SLURM_JOB_ID/a.sock"
completion_marker="$raw/${prefix}_completion_marker.json"
cleanup() {
...
if x['iterations'] != 200 or x['deadline_misses']:
    raise SystemExit('confirm81 operational requalification failed')
print('confirm81 operational requalification passed', flush=True)
PY
...
shifter_gpu env CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=20 CUDA_MPS_CLIENT_PRIORITY=1 \
    python3 /softwall/ai_worker_fenced.py --kind nrx --mode rpc \
    --repeats 1 --socket "$background_socket" \
    --completion-marker "$completion_marker" --joint-response-delay-ms 25 \
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/four_cell_fenced_fault_controller.py \
...
cleanup
trap - EXIT INT TERM
echo "confirm81 fenced AI timeout four-cell run completed"
