...
"""Qualified-duration conditional-capacity screen with seven visible AI units.
...
def exact_joint(state: State) -> dict:
    """One subset DP over mandatory recovery and optional visible AI jobs."""
    jobs = state.recovery + state.ai
    mandatory_mask = (1 << len(state.recovery)) - 1
    full = (1 << len(jobs)) - 1
    states: dict[int, tuple[int, tuple[tuple[int, int, int], ...]]] = {
        0: (state.now_ms, ())
    }
    ...
    for mask in range(full + 1):
        ...
        if mask & mandatory_mask == mandatory_mask:
            names = sorted(
                job.name for index, job in enumerate(jobs)
                if index >= len(state.recovery) and mask & (1 << index)
            )
            value = sum(
                job.value for index, job in enumerate(jobs)
                if index >= len(state.recovery) and mask & (1 << index)
            )
            if value > best_value or (value == best_value and names < best_names):
                best_value, best_names, best_path = value, names, path
        for index, job in enumerate(jobs):
            if mask & (1 << index):
                continue
            start = max(cursor, job.release_ms)
    ...
    return {
        ...
        "schedule": [
            {"job": jobs[index].name, "start_ms": start, "finish_ms": finish}
            for index, start, finish in best_path
        ],
    }
...
class Problem:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.cells) > 4 or len(self.ai) > 7:
            raise ValueError("screen limited to at most four cells and seven AI units")
...
def main() -> None:
    ...
    report = {
        ...
        "scope": "At most four radio TBs, seven visible AI jobs, one early AI unit, and two stages; prewarmed GC-OFF four-cell sample-qualified durations. This is not an online GPU policy or deadline proof.",
        "exact_joint": exact,
