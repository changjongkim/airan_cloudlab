...
"""Frozen audit of observe-first NRx45/conv12/AI50 Qwen four-cell calendar."""
...
def main() -> None:
    ...
    protocol = read(base / "confirm100_observe_first_qwen_protocol.json")
    ...
    for arm in protocol["arms"]:
        ...
        two_admitted = sum(sum(row["admitted"] for row in group) == 2
                           for group in groups)
        one_admitted = sum(sum(row["admitted"] for row in group) == 1
                           for group in groups)
        ...
        gates = {
            "contract": (
                ...
                and ran["nrx_bound_ms"] == 45
                and ran["conv_bound_ms"] == 12
                and ran["ai_budget_ms"] == 50
                and ran["ai_rpc_timeout_ms"] == 45
            ...
            "sample_safety": (
                ...
                and max(nrx) <= 45 and max(conv) <= 12
                and max(ai_host) <= 50
            ...
            "coverage": (
                ...
                and one_admitted >= protocol["min_one_admitted"]
        ...
        arms.append({
            ...
            "one_admitted_releases": one_admitted,
    ...
    report = {
        "schema": "softwall-confirm100-observe-first-qwen-fourcell-v1",
        ...
        "interpretation": "Observe-first NRx45/conv12/AI50 four-cell calendar with Qwen2.5-1.5B b1c16 prefill cap20 and continuous backlog. This candidate was selected after Confirm99 failed NRx45 due to late controller observation, using new seeds and a source-separated controller. Fewer-than-seven Qwen returns by D153 under this controller show observed demand pressure, not optimal schedule infeasibility. Passing is finite prewarmed GC-OFF sample, not WCET, online seven-job lease, production DU, or policy superiority.",
    }
    output = base / f"confirm100_observe_first_qwen_job{protocol['job']}.json"
