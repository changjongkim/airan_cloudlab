...
"""Gate one frozen C158 repeated actual-NeuralRx qualification run."""
...
def main() -> None:
    ...
    activations = {
        int(row["sequence"]): row for row in nrx.get("activations", [])
    }
    ...
    for key, record in owner_records:
        ...
        activation = activations.get(sequence)
        staging_consistent &= (
            activation is not None
            and record.get("preparation_completed_ns", 2**63)
            <= activation.get("started_ns", -1)
            and activation.get("completed_ns", 2**63)
    ...
    gates = {
        ...
        "declared_staging_and_activation": (
            staging_consistent and len(activations) == iterations
            and all(row.get("host_ms", float("inf"))
                    < mode["release_prewarm_lead_ms"]
    ...
    value = {
        "schema": "softwall-confirm158-repeated-result-v1",
        "status": (
            "C158_REPEATED_ACTUAL_NRX_PASS" if all(gates.values())
            else "C158_REPEATED_ACTUAL_NRX_FAIL"
        ),
