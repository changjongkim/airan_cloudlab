...
def composed_ai_guard_ns(base_guard_ns: int, broker_rpc_timeout_ms: float,
                         global_broker_enabled: bool) -> tuple[int, int]:
    """Reserve prepare+commit+complete RPC budgets around one physical AI unit."""
    broker_budget_ns = (
        3 * round(broker_rpc_timeout_ms * 1e6)
        if global_broker_enabled else 0
    )
...
def complete_selected(queue, request, returned_ns):
    ...
    if getattr(queue, "completion_ack_deferred", False):
        # The physical completion is confirmed locally, but the global token
        # intentionally remains unavailable until the control worker gets ACK.
        return False
    return True if result is None else bool(result)
...
def main() -> None:
    ...
    parser.add_argument("--global-broker-rpc-timeout-ms", type=float, default=5)
    ...
    if args.global_broker_rpc_timeout_ms <= 0:
        parser.error("global broker RPC timeout must be positive")
    if min(
    ...
    try:
        ...
        (admission_ai_guard_ns,
         global_broker_transaction_budget_ns) = composed_ai_guard_ns(
            ai_guard_ns, args.global_broker_rpc_timeout_ms,
            args.global_trace_broker is not None,
        )
        ...
        trace_queue = (
            DeadlineFaultContainedGlobalTraceClient(
                ...
                args.global_broker_rpc_timeout_ms,
            )
        ...
        for index in range(args.iterations):
            ...
            if any(tx.reservation is not None for tx in transactions):
                ...
                async_ai_request = select_request(
                    ...
                    admission_ai_guard_ns, args.ai_bound_map,
                )
            ...
            if any(tx.reservation is None for tx in transactions):
                finish_pre_ai(
                    ...
                    async_ai_request, async_ai_cutoff_ns, admission_ai_guard_ns,
                )
                async_ai_thread = None
            finish_pre_ai(
                async_ai_thread, async_ai_holder, background, trace_queue,
            ...
            if pending:
                ...
                if background.enabled and request is not None:
                    # A pipelined prepare may have completed after the earlier
                    # timestamp.  Replan from a fresh decision instant; the
                    # client also revalidates the absolute horizon before
                    # returning the token.
                    now_ns = time.perf_counter_ns()
                    ...
                    if args.system == "softwall":
                        ...
                        joint = runtime.replan_and_lease(
                            moves, "ai", now_ns,
                            request["deadline_ns"] - now_ns, admission_ai_guard_ns,
                            [BackgroundUnit(
                    ...
                    if args.system == "softwall" and joint is not None:
                        ...
                        ai_record = execute_request(
                            ...
                            earliest_recovery_ns, admission_ai_guard_ns,
                        )
                        ...
                        execute_request(
                            background, trace_queue, request,
                            "after_nrx_before_recovery", args.ai_bound_map,
                            horizon_ns, admission_ai_guard_ns,
                        )
                        recovery_action["ai_completed_before_recovery"] = True
                    else:
            ...
            while background.enabled:
                ...
                execute_request(
                    ...
                    args.ai_bound_map, next_release_ns, admission_ai_guard_ns,
                )
    ...
    result = {
        "schema": "softwall-global-trace-home-fully-budgeted-fault-contained-v1",
        "system": args.system,
        ...
        "global_broker_rpc_timeout_ms": args.global_broker_rpc_timeout_ms,
        ...
        "global_broker_transaction_budget_ms": global_broker_transaction_budget_ns / 1e6,
        "admission_ai_guard_ms": admission_ai_guard_ns / 1e6,
