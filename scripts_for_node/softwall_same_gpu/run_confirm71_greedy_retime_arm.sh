...
# Confirm71: AI-aware greedy recovery retime versus fixed recovery arm.
...
require_allocation
mode=${1:?mode must be off or on}
case "$mode" in off|on) ;; *) echo "invalid mode: $mode" >&2; exit 2 ;; esac
payload_seed=${SOFTWALL_PAYLOAD_SEED:-20710001}
channel_seed_base=${SOFTWALL_CHANNEL_SEED_BASE:-20711000}
iterations=${SOFTWALL_GATE_ITERATIONS:-200}
prefix=${SOFTWALL_GATE_PREFIX:-confirm71_${mode}_seed${channel_seed_base}_job${SLURM_JOB_ID}}
...
if x['iterations'] != 200 or x['deadline_misses']:
    raise SystemExit('confirm71 operational requalification failed')
print('confirm71 operational requalification passed', flush=True)
PY
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/two_endpoint_greedy_retime_controller.py \
...
cleanup
trap - EXIT INT TERM
echo "confirm71 mode=$mode completed"
