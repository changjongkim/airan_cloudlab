...
# Confirm72: isolate recovery credit retime from pre-recovery AI admission.
...
require_allocation
mode=${1:?mode must be lease_only or on}
case "$mode" in lease_only|on) ;; *) echo "invalid mode: $mode" >&2; exit 2 ;; esac
payload_seed=${SOFTWALL_PAYLOAD_SEED:-20720001}
channel_seed_base=${SOFTWALL_CHANNEL_SEED_BASE:-20721000}
iterations=${SOFTWALL_GATE_ITERATIONS:-200}
prefix=${SOFTWALL_GATE_PREFIX:-confirm72_${mode}_seed${channel_seed_base}_job${SLURM_JOB_ID}}
...
if x['iterations'] != 200 or x['deadline_misses']:
    raise SystemExit('confirm72 operational requalification failed')
print('confirm72 operational requalification passed', flush=True)
PY
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/two_endpoint_retime_ablation_controller.py \
    ...
    --ai-aware-recovery "$mode" --ai-deadline-ms 50
...
cleanup
trap - EXIT INT TERM
echo "confirm72 mode=$mode completed"
