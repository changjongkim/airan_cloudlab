...
# V16 qualification arm: inject abort post-apply reply loss without changing frozen V15 sources.
...
label=${SOFTWALL_SHARD_LABEL:-v16_abort_fault_job${SLURM_JOB_ID}}
...
crash_operation=${SOFTWALL_BROKER_CRASH_OPERATION:-abort}
crash_number=${SOFTWALL_BROKER_CRASH_NUMBER:-3}
case "$crash_operation" in abort) ;; *) echo "V16 arm only accepts abort" >&2; exit 2;; esac
...
shifter_home_aerial 0 python3 /softwall/four_point_crash_global_trace_lease_broker.py \
...
gates={
 ...
 'target_execution_semantics':(
     ...
     else request_ids.count(target_request_id)==0 if operation in ('abort','commit')
 ...
 'rpc_telemetry_present':len(rpc_by_home)==2 and all(value is not None and value.get('schema')=='softwall-pipelined-broker-rpc-telemetry-v1' for value in rpc_by_home),
 'single_unlaunched_token':all(value.get('ownership_contract')==p['ownership_contract'] and value.get('ownership_evidence',{}).get('maximum_unlaunched_tokens',2)<=1 and value.get('state',{}).get('ownership_invariant_holds') for value in rpc_by_home),
 'retained_token_branch_exercised':all(value.get('ownership_evidence',{}).get('suppressed_prepare_due_owned_token',0)>0 for value in rpc_by_home),
 'socket_and_admission_bounds_distinct':p['global_broker_rpc_timeout_ms']<p['global_broker_rpc_admission_bound_ms'] and all(c['global_queue_status']['broker_rpc_socket_timeout_ms']==p['global_broker_rpc_timeout_ms'] and c['global_queue_status']['broker_commit_admission_bound_ms']==p['global_broker_rpc_admission_bound_ms'] for c in controllers),
 'rpc_all_four_operations_each_home':all({'prepare','abort','commit','complete'}<=ops for ops in rpc_ops_by_home),
 ...
 'commit_admission_wall_bound':bool([row for row in rpc_records if row['operation']=='commit']) and all(row['elapsed_ms']<=p['global_broker_rpc_admission_bound_ms'] for row in rpc_records if row['operation']=='commit'),
}
value={'schema':'softwall-v16-abort-fault-arm-result-v1','protocol':str(protocol),'crash_operation':operation,'broker_exit_status':broker_status,'fault_detected_ns':fault_detected_ns,'target_request_id':target_request_id,'operational_global_queue_faults':operational_faults,'radio_records_after_fault_detection':radio_after,'homes':homes,'control_evidence':rpc_control,'gates':gates,'all_pass':all(gates.values()),'artifact_sha256':{str(path):hashlib.sha256(path.read_bytes()).hexdigest() for path in artifacts},'scope':f'V16 qualification: post-apply home0 {operation}{p["fault_injection"]["operation_number"]} fail-stop with asynchronous prepare/abort/complete and one 7 ms launch-commit admission charge; local RAN certificates continue while ambiguous global state remains quarantined'}
tmp=out.with_suffix('.tmp');tmp.write_text(json.dumps(value,indent=2));tmp.replace(out);print(json.dumps(gates,indent=2))
if not value['all_pass']:raise SystemExit('v16 abort fault arm failed')
PY
cleanup
trap - EXIT INT TERM
echo "v16 abort fault arm complete: $result"
