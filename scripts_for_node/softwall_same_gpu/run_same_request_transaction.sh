...
fallback_timing=${FALLBACK_TIMING:-latest}
...
[[ "$fallback_timing" == latest || "$fallback_timing" == early_if_clear ]] || {
    echo 'FALLBACK_TIMING must be latest or early_if_clear' >&2
    exit 2
}
...
shifter_gpu env CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/same_request_transaction_controller.py \
    --tag "$tag" --ipc-dir "$state" \
    --engine /softwall_runtime/engines/neural_rx_fp16_full.trt \
    ...
    --fallback-timing "$fallback_timing" \
    "${controller_args[@]}"
