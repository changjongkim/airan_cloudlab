#!/bin/bash
# Run ONLY Phase C RDMA variants (Configs 4 and 7) with Topology B
# (3g+2g+2g).
...
RDMA_ARGS=(--network=host --cap-add=IPC_LOCK --device=/dev/infiniband --ipc=host)
...
cleanup_rdma_tag() {
    ...
    sudo find /dev/shm -maxdepth 1 -type f \
        -name "rdma_${tag}_*.info" -delete 2>/dev/null || true
}

cleanup_containers() {
    docker rm -f \
        airan_c4_rdma_l1 airan_c4_rdma_nrx airan_c4_rdma_qwen \
        airan_c7_rdma_l1 airan_c7_rdma_nrx airan_c7_rdma_qwen \
        > /dev/null 2>&1 || true
    cleanup_rdma_tag config4_rdma
    cleanup_rdma_tag config7_rdma
}
...
run_rdma_config() {
    ...
    local l1_name="airan_${prefix}_rdma_l1"
    local nrx_name="airan_${prefix}_rdma_nrx"
    local qwen_name="airan_${prefix}_rdma_qwen"
    local info_file="/dev/shm/rdma_${tag}_fwd_cons.info"
    local l1_rc nrx_wait_rc nrx_rc

    step "$description"
    mkdir -p "$outdir"; chmod 777 "$outdir"
    cleanup_rdma_tag "$tag"
    docker rm -f "$l1_name" "$nrx_name" "$qwen_name" > /dev/null 2>&1 || true

    # Warm the AI workload before establishing the two RDMA QPs. Starting
    # NRx first and waiting for its ready line would deadlock: RdmaEndpoint
    ...
    if ! docker run -d --name "$nrx_name" --gpus "\"device=$MIG_NRX\"" \
        ...
        python3 nrx_consumer.py "$tag" > /dev/null 2>&1; then
        collect_logs "$outdir" "$nrx_name" "$qwen_name"
    ...
    docker run --rm --name "$l1_name" --gpus "\"device=$MIG_L1\"" \
        ...
        python3 l1_producer.py "$label" "$L1_ITERS" "$tag" > "$outdir/l1.log" 2>&1
    l1_rc=$?
    ...
    if ! grep -q '\[NRx\] ready' "$outdir/nrx.log"; then
        alert "$cfg: NRx ready marker missing"
        return 1
    fi
    if ! grep -q '\[NRx\] shutdown signal' "$outdir/nrx.log"; then
        alert "$cfg: NRx shutdown marker missing"
        return 1
    fi
...
on_exit() {
    ...
    if [ "$TOPOLOGY_NEEDS_RESTORE" -eq 1 ]; then
        if restore_topology_A; then
            ...
            alert "PHASE C exit cleanup could not restore Topology A"
            rc=1
        fi
    fi
    exit "$rc"
...
trap on_exit EXIT
trap 'alert "PHASE C stopped by SIGINT"; exit 130' INT
trap 'alert "PHASE C stopped by SIGTERM"; exit 143' TERM

log "==== PHASE C RDMA start (Topology B: 3g+2g+2g) ===="
for cfg in config4_rdma config7_rdma; do
...
if [ -n "${MIG_L1:-}" ] && [ -n "${MIG_NRX:-}" ] && [ -n "${MIG_AI:-}" ]; then
    run_rdma_config \
        config4_rdma config4_rdma_l1 config4_rdma config4_cross_mig_rdma c4 \
        "Config 4 RDMA: L1(2g)<->NRx(2g) + Qwen(3g)"
    rc4=$?
    run_rdma_config \
        config7_rdma config7_rdma_l1 config7_rdma config7_cross_repeat_rdma c7 \
        "Config 7 RDMA: cross-partition repeat + Qwen(3g)"
    rc7=$?
    if [ "$rc4" -ne 0 ] || [ "$rc7" -ne 0 ]; then
        alert "PHASE C RDMA failed (config4_rc=$rc4 config7_rc=$rc7)"
        exit 1
    fi
else
