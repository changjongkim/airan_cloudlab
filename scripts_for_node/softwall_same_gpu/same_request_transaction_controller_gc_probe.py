...
def main() -> None:
    ...
    parser.add_argument("--gc-mode", choices=("on", "off"), default="on")
    ...
    def gc_probe(phase, info):
        ...
        if phase == "start":
            gc_start_ns[generation] = now_ns
        else:
            gc_counts[generation] += 1
            started_ns = gc_start_ns.pop(generation, now_ns)
            duration_ms = (now_ns - started_ns) / 1e6
            if duration_ms >= 1.0:
                gc_long_events.append({
                    "generation": generation,
                    "release_index_at_callback": gc_epoch_index[0],
    ...
    try:
        ...
        # Both modes collect before the epoch. Only the intervention suppresses
        # automatic collections while requests are timed; the callback records
        # collections >=1 ms in the control condition.
        gc.collect()
        ...
        for index in range(args.iterations):
            ...
            if commit_kind != "nrx":
                fallback = admitted
                if admitted:
                    # The recovery calendar owns this interval. Bounded AI may
                    # run before it only if its complete RPC fits the gap.
                    if args.recovery_gap_ai and background is not None:
                        ...
                        while (
                            ...
                            background.run(index, fallback_start_ns, args.ai_budget_ms)
                        gap_ai_units = len(background.records) - before
                    wait_until(fallback_start_ns)
                    runtime.start_fallback(transaction, time.perf_counter_ns())
    ...
    result = {
        ...
        "gc_counts_by_generation": gc_counts,
        "gc_long_events": gc_long_events,
