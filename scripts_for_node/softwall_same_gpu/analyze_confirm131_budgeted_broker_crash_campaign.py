#!/usr/bin/env python3
"""Aggregate prospective C131 admission-budgeted broker fail-stop arms."""
...
def main() -> None:
    ...
    gates = {
        ...
        "control_budget_accounted_in_admission": all(
            ...
            and all(home["global_broker_transaction_budget_ms"] == 10
                    and home["admission_ai_guard_ms"] == 12
    ...
    value = {
        "schema": "softwall-confirm131-budgeted-broker-crash-campaign-v1",
        ...
        "claim": (
            "With 5 ms per-RPC bounds charged as a 10 ms commit+complete admission budget, after a broker fail-stop "
    ...
    if not value["all_pass"]:
        raise SystemExit("C131 campaign gate failed")
