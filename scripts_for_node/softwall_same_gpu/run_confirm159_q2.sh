...
# C159-Q2 variable-context P180 actual TensorRT NeuralRx qualification.
...
label=${SOFTWALL_C159_LABEL:-confirm159q2_variable_job${SLURM_JOB_ID}}
campaign=${SOFTWALL_C159_CAMPAIGN:-development}
seed_base=${SOFTWALL_C159_SEED_BASE:-34000000}
excluded_nodes=${SOFTWALL_C159_EXCLUDED_NODES:-nid002288,nid002100,nid001109,nid001085,nid001064,nid001124,nid001177,nid001308}
iterations=${SOFTWALL_C159_ITERATIONS:-600}
minimum_executed_per_class=${SOFTWALL_C159_MIN_EXECUTED_PER_CLASS:-50}
...
python3.11 "$SOFTWALL_SCRIPTS/build_confirm159_q2_protocol.py" \
    ...
    --minimum-executed-per-class "$minimum_executed_per_class" \
...
case ",$excluded_nodes," in
    *",$current_node,"*)
        echo "C159-Q2 node $current_node is in the frozen exclusion set" >&2
        exit 3
        ;;
esac
...
shifter_all env CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=80 CUDA_MPS_CLIENT_PRIORITY=0 \
    python3 /softwall/c159_q2_coordinator.py \
    --peer-spec "$peer_spec" --ipc-dir "$state" \
...
python3.11 "$SOFTWALL_SCRIPTS/analyze_confirm159_q2.py" \
...
cleanup
trap - EXIT INT TERM
echo "C159-Q2 variable-context P180 qualification complete: $result"
