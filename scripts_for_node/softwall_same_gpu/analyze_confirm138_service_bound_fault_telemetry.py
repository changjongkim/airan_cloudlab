#!/usr/bin/env python3
"""Aggregate C138 broker fail-stop attempted-RPC telemetry arms."""
...
CONTAINER_SOURCE_MAP = {
    ...
    "/softwall/global_trace_service_bound_fault_controller.py":
        "scripts_for_node/softwall_same_gpu/global_trace_service_bound_fault_controller.py",
...
def summarize_rpc_records(arm_results, timeout_ms):
    counts = Counter()
    ...
    wall_exceeded = sum(bool(row.get("wall_timeout_exceeded")) for row in records)
    gates = {
        ...
        "no_wall_timeout_exceedance": wall_exceeded == 0,
    ...
    return {
        ...
        "wall_timeout_exceeded": wall_exceeded,
        "gates": gates,
...
def analyze(campaign_path, arm_paths):
    ...
    return {
        "schema": "softwall-confirm138-service-bound-fault-telemetry-v1",
        ...
        "claim_boundary": (
            ...
            "and complete broker fail-stop detection calls under a prospective 5 ms "
            "per-RPC budget on one new A100 node. This is not WCET, production d_MAC, "
...
def main():
    ...
    if not result["all_pass"]:
        raise SystemExit("C138 service-bound fault telemetry gate failed")
