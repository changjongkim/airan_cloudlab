...
# Two four-cell RAN homes consuming one global AI trace with one injected
# post-commit broker reply loss and fail-closed per-home containment.
...
label=${SOFTWALL_SHARD_LABEL:-global_ai_broker_fault_job${SLURM_JOB_ID}}
...
python3 - "$protocol" "$iterations" "$warmup" "$label" "$global_policy" "$conv_bound_ms" \
    "$SOFTWALL_SCRIPTS/global_trace_fault_contained_controller.py" \
    "$SOFTWALL_SCRIPTS/fault_contained_global_trace_client.py" \
    "$SOFTWALL_SCRIPTS/fault_inject_global_trace_lease_broker.py" \
...
keys=["/softwall/global_trace_fault_contained_controller.py","/softwall/fault_contained_global_trace_client.py","/softwall/fault_inject_global_trace_lease_broker.py","/softwall/same_request_ipc_worker.py","/softwall/trace_qwen_worker.py","/softwall_task1/isca_v2/dart_runtime.py","/softwall_task1/isca_v2/cuda_ipc_channel.py"]
paths=[Path(value) for value in sys.argv[7:]]
value={"schema":"softwall-global-ai-broker-fault-arm-protocol-v1","status":"frozen-before-run","label":label,"iterations":iterations,"warmup":warmup,"global_policy":policy,"placement":{"gpu0":"four-cell home + two local NRx + Qwen","gpu1":"four-cell home + two local NRx + Qwen"},"shared_resources":"one serializable BurstGPT request queue; recovery, NRx endpoint/IPC, Qwen execution and physical credits remain per-home","transaction_order":"observe NRx -> certificate order -> global hold -> local atomic replan+AI lease -> global commit; ambiguous reply disables that home global AI without retry; RAN continues","fault_injection":{"home":0,"commit_number":30,"point":"after broker applies commit, before reply"},"cuda_module_loading":"LAZY","period_ms":180,"deadline_ms":155,"nrx_bound_ms":45,"conv_bound_ms":conv_bound_ms,"guard_ms":2,"fault_every":5,"source_sha256":{key:hashlib.sha256(path.read_bytes()).hexdigest() for key,path in zip(keys,paths)}}
...
shifter_home_aerial 0 python3 /softwall/fault_inject_global_trace_lease_broker.py \
    ...
    --participants 2 --policy "$global_policy" --drop-commit-response-home 0 \
    --drop-commit-response-number 30 --output "$broker_output" &
broker_pid=$!
...
for home in 0 1; do
    ...
    shifter_home_aerial "$home" env CUDA_MPS_CLIENT_PRIORITY=0 \
        python3 /softwall/global_trace_fault_contained_controller.py \
...
homes=[];artifacts=[protocol,broker_path];request_ids=[];controllers=[]
for home in (0,1):
...
faults=broker['faults'];fault=faults[0] if len(faults)==1 else {}
orphan_id=fault.get('request_id');fault_ns=fault.get('injected_ns',2**63-1)
home0_faults=controllers[0]['global_queue_status']['faults']
home1_after_fault=sum(row['admitted_ns']>fault_ns for row in controllers[1]['background_records'])
orphan=[row for row in broker['requests'] if row['request_id']==orphan_id]
gates={
 ...
 'broker_homes':summary['registered_homes']==[0,1] and summary['finalized_homes']==[0,1],
 'one_injected_fault':len(faults)==1 and fault.get('home')==0 and fault.get('commit_number')==30,
 'home0_fail_closed':not controllers[0]['global_queue_status']['enabled'] and len(home0_faults)==1 and home0_faults[0]['operation']=='commit' and home0_faults[0]['request_id']==orphan_id,
 'home1_unaffected':controllers[1]['global_queue_status']['enabled'] and not controllers[1]['global_queue_status']['faults'] and home1_after_fault>0,
 'orphan_isolated':len(orphan)==1 and orphan[0]['state']=='inflight' and orphan_id not in request_ids,
 'no_duplicate_execution':len(request_ids)==len(set(request_ids))==summary['timely_requests'],
 'expected_broker_residue':summary['outstanding_tokens']==1 and summary['duplicate_commit_count']==0,
 'both_homes_executed_ai':all(h['summary']['ai_timely_value_tokens']>0 for h in homes),
}
value={'schema':'softwall-global-ai-broker-fault-arm-result-v1','protocol':str(protocol),'broker':str(broker_path),'broker_summary':summary,'broker_faults':faults,'orphan_request':orphan,'home1_ai_after_fault':home1_after_fault,'homes':homes,'gates':gates,'all_pass':all(gates.values()),'artifact_sha256':{str(path):hashlib.sha256(path.read_bytes()).hexdigest() for path in artifacts},'scope':'post-commit response loss at home0; ambiguous request quarantined; home0 RAN and home1 RAN+AI continue under local recovery certificates'}
tmp=out.with_suffix('.tmp');tmp.write_text(json.dumps(value,indent=2));tmp.replace(out);print(json.dumps(gates,indent=2))
if not value['all_pass']:raise SystemExit('global AI broker fault-containment arm failed')
PY

cleanup
trap - EXIT INT TERM
echo "global AI broker fault-containment arm complete: $result"
