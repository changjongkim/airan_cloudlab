#!/usr/bin/env bash
# Chain 18 Part 2 — NCU on Full GPU (Config B) with --clock-control none
# Fixes MIG clock-lock limitation from Chain 17 Part C
set -uo pipefail
...
OUT=/mydata/results/$DATE_DIR/chain18_p2_ncu
...
# NCU-profiled L1 run on Full GPU (no clock-lock issue since not MIG)
profile_l1_ncu() {
  local label=$1
...
smsp__inst_executed.sum,gpu__time_active.sum"
  docker run --rm --user 0:0 --gpus "device=$UUID_L1" \
    ...
    bash -c "ncu --target-processes all --launch-count $NCU_KERNELS \
      ...
      python3 real_l1.py $label $CELLS $L1_ITERS 2>&1 | tail -3" > "$OUT/${label}.ncu.stdout" 2>&1
}
