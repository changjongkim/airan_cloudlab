#!/usr/bin/env python3
"""Evaluate the frozen same-node BAAB recovery-gap lease replication."""
...
def main() -> None:
    ...
    requal = read(args.raw / f"confirm44_requalification_job{args.job}.json")
    conditions: list[tuple[str, dict, dict, dict]] = []
    for index, mode in enumerate(("on", "off", "off", "on"), start=1):
        prefix = args.raw / f"confirm44_gap_r{index}_{mode}_cap80_job{args.job}"
    ...
    gates: dict[str, bool] = {}
    gates["frozen_reverse_order"] = protocol["execution"]["order"] == [
        "gap_on", "gap_off", "gap_off", "gap_on"
    ]
    ...
    # In BAAB order, match each ON run to its closest OFF run.
    pairs = [paired(conditions[0][1], conditions[1][1]), paired(conditions[3][1], conditions[2][1])]
    ...
    result = {
        "schema": "softwall-confirm44-recovery-gap-baab-v1",
        "job": args.job,
