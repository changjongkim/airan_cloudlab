#!/usr/bin/env python3
"""Variable-length Qwen worker for C161 phase-2 response/fence faults.
...
from c161_phase2_faults import marker_paths
...
class TraceQwenPrefill:
    ...
    def submit(self, context_length: int):
...
def main() -> None:
    ...
    parser.add_argument("--fault-response-delay-ms", type=float, default=25.0)
    args = parser.parse_args()
    if (args.batch_size <= 0 or args.warmup_per_length < 0
            or args.fault_response_delay_ms <= 0):
    ...
    try:
        ...
        with connection, connection.makefile("rwb", buffering=0) as channel:
            while True:
                ...
                try:
                    ...
                    if fault_mode == "pre_fence_channel_loss":
                        # Close the only controller channel after physical CUDA
                        # submission and before any completion fence is exposed.
                        # The worker privately drains the kernel for artifact
                        # integrity but intentionally publishes no completion
                        # marker. The controller must retain the lease.
                        connection.shutdown(socket.SHUT_RDWR)
                        gpu_ms = unit.finish(events)
                        ...
                        fault_events.append(record)
                        break
                    gpu_ms = unit.finish(events)
                    completed_ns = time.perf_counter_ns()
                    marker = {
                        "schema": "softwall-c161-qwen-completion-v1",
                        "lease_id": request_id,
                        "context_length": context_length,
                        "accepted_ns": accepted_ns,
                        "submitted_ns": submitted_ns,
                    ...
                    atomic_json(completion_path, marker)
                    record = {
                        "request_id": request_id,
                        "context_length": context_length,
                        "latest_start_ns": latest_start_ns,
                    ...
                    response = {
                        ...
                        "unit": len(records) + 1, **record,
                    }
                    if fault_mode == "post_fence_reply_delay":
                        time.sleep(args.fault_response_delay_ms / 1000.0)
                    response_sent = True
                    try:
                        channel.write(json.dumps(response).encode() + b"\n")
                    except (BrokenPipeError, ConnectionError, OSError):
                        response_sent = False
                    record["response_sent"] = response_sent
                    records.append(record)
                    if fault_mode == "post_fence_reply_delay":
                        fault_events.append(record)
                        break
        ...
        result = {
            "schema": "softwall-c161-phase2-qwen-prefill-v1",
            "model": args.model,
            "phase": "prefill",
            ...
            "fault_response_delay_ms": args.fault_response_delay_ms,
