...
EN = {
    ...
    "(a) 배치마다 대기열이 무너지는 지점이 다름": "(a) Queue-collapse points differ by placement",
    "전체 GPU 공유(MPS)": "Full-GPU sharing (MPS)",
    "MIG 안에 L1+NRx": "L1+NRx inside MIG",
    "MIG+MPS 안에 L1+NRx": "L1+NRx inside MIG+MPS",
    "L1·NRx 분리(GPU 직접/P2P)": "Separate L1/NRx (GPU-direct/P2P)",
    "L1·NRx 분리(NIC 직접/GDR)": "Separate L1/NRx (NIC-direct/GDR)",
    ...
    "(b) 이번 실험에서 안정적으로 처리한 범위": "(b) Stable range in this experiment",
    "같은 요청률 비교: 격리 방식마다 한계는 다르지만 고정된 NRx 경로는 모두 처리 상한이 있음":
        "Equal-rate comparison: every fixed NRx path has a capacity ceiling",
    "실험 범위: 다른 AI 작업 없이 초당 50~350개 요청, 10초 x 3회 x 5개 배치 = 120회. 100 ms는 대기열 붕괴 확인선":
        "Scope: 50-350 requests/s without background AI; 10 s x 3 trials x 5 placements = 120 runs; 100 ms marks queue collapse",
